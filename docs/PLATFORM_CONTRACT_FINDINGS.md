...
CORS allowlists: telemetry + auth default `https://play.brewser.io` (`switch-telemetry.php:636`, `brewser-auth.php:92`); submissions `https://my.brewser.io` + `https://play.brewser.io` (`class-brewser-sub-rest.php:33-34`).
...
- Default play base: `'https://natureglass.github.io/Brewser-apps/apps'`, option `brewser_apps_settings.play_base_url` (`brewser-apps.php:52`); admin note says intended move to `https://play.brewser.io/apps` (`class-brewser-settings.php:98`).
...
CNAME = play.brewser.io       .nojekyll present
(REMOVED 2026-08-01: categories.json, tags.json, permissions.json — unused; see below)
(versions.json MOVED to brewser-apps-staging → my.brewser.io — see §3)
```
...
- `versions.json` — **MOVED to `brewser-apps-staging`** (served at `my.brewser.io/versions.json`). Its sole producer is now `brewser`'s `make release` mirror of `romfs/configs/current.json`; the old `scripts/collect_versions.py` generator (and the `brewser-apps` Makefile `versions` target) was retired.
...
- `index.html` = crawler redirect to `https://brewser.io/` (`index.html:9-21`), noindex.

### 2.4 Hosting

`CNAME` = `play.brewser.io` (`CNAME:1`). A catalogue entry maps to `https://play.brewser.io/apps/<id>/<entry>`; logo `…/apps/<id>/<logo>`; artifacts `…/artifacts/<id>.json`; catalogue `…/catalogue.json`. (Deduced from CNAME + layout; no consumer code lives in this repo.) The same content is fetchable via `raw.githubusercontent.com/natureglass/Brewser-apps/refs/heads/main/…`, which is what the runtime pins today (§4).
...
**Versions check:** fetch `versions.json` (now `my.brewser.io`, brewser-apps-staging), compare string-inequality per key against seeded `configs/current.json` (`updates-modal.js:374-455`). File has exactly the three expected keys — **compatible**. (`config.json:18` `checkVerOnBoot` is an orphan — parsed nowhere.)
...
1. **Canonical runtime base URL:** should the pinned catalogue/app URLs move from `raw.githubusercontent.com/natureglass/Brewser-apps/…` to `https://play.brewser.io/…` (CNAME is live in the repo; the WP option still defaults to `natureglass.github.io`)? Which of the three hosts is the contract?
