...
3. **Context boot path.** Locate where the single V8 context is created, where globalThis is populated, and where the runtime bundle is injected, in both engine and `brewser-runtime-v8` boot code.
...
- **Runtime (`brewser-runtime-v8`):** demo context creation hooks into the Phase A lifecycle (V8 context + GL context + OwnershipRegistry now form the single composite teardown on `endAppSession`); routing shims install per new GL context (their per-GL-object gates now naturally reset).
