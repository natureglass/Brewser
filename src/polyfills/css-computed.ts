...
import { getComputedLiveStyle, type ComputedLiveStyle } from '../scripts/live-css.js';

/** Read `<number>px` (or unitless number) as a string. CSS spec
 * always serialises lengths with the `px` unit attached. */
function serializePx(n: number | undefined): string {
...
function extractBackgroundColor(bg: string): string {
	if (!bg) return '';
...
class LiveCSSStyleDeclaration {
	...
	constructor(computed: ComputedLiveStyle) { this.#computed = computed; }
	...
	getPropertyValue(name: string): string {
		...
		switch (name.toLowerCase()) {
			...
			case 'width': return serializePx(c.width as number | undefined);
			case 'height': return serializePx(c.height as number | undefined);
	...
	get background(): string { return this.getPropertyValue('background'); }
	get backgroundColor(): string { return this.getPropertyValue('background-color'); }
	get fontFamily(): string { return this.getPropertyValue('font-family'); }
...
export function installGetComputedStyle(): void {
	if (installed) return;
	installed = true;
	const fn = (el: unknown, _pseudo?: string | null): LiveCSSStyleDeclaration => {
		// Pseudo-element second arg is accepted but ignored (Tier-1).
		// Returns an "empty" declaration for null / undefined elements.
		const computed = el ? getComputedLiveStyle(el as LiveElement) : {};
