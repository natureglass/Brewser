...
const allObservers: MutationObserverImpl[] = [];
let microtaskScheduled = false;

class MutationObserverImpl {
	#callback: (records: MutationRecord[], observer: MutationObserverImpl) => void;
	...
	observe(target: unknown, options: ObserverOptions = {}): void {
		if (target == null) {
		...
		if (!attrs && !cdata && !childList) {
			throw new TypeError(
				'MutationObserver.observe: must enable at least one of childList, attributes, characterData',
			);
		}
		...
		else this.#registrations.push({ target, options: normalized });
		if (allObservers.indexOf(this) < 0) allObservers.push(this);
	}
	...
	disconnect(): void {
		...
		if (i >= 0) allObservers.splice(i, 1);
	}
	...
	_consider(target: unknown, record: MutationRecord): void {
		for (const reg of this.#registrations) {
			if (!this._matches(reg, target, record)) continue;
	...
	_matches(
		...
		if (reg.target !== target) {
			...
			if (!isAncestor(reg.target, target)) return false;
		}
		if (record.type === 'childList' && !reg.options.childList) return false;
		if (record.type === 'attributes' && !reg.options.attributes) return false;
	...
	_drain(): void {
		...
		try {
			...
			console.debug(
				'[MutationObserver] callback threw: ' + (error as Error).message,
			);
		}
	}
}

function isAncestor(maybeAncestor: unknown, target: unknown): boolean {
	let n: unknown = (target as { parent?: unknown; parentNode?: unknown } | null)
		?.parent ??
		(target as { parentNode?: unknown } | null)?.parentNode ??
		null;
	while (n) {
		if (n === maybeAncestor) return true;
		n = (n as { parent?: unknown; parentNode?: unknown }).parent ??
			(n as { parentNode?: unknown }).parentNode ??
			null;
	}
	return false;
}

function scheduleMicrotaskDrain(): void {
	...
	Promise.resolve().then(() => {
		...
		for (const obs of snapshot) obs._drain();
	});
}
...
export function notifyChildList(
	...
	for (const obs of allObservers) obs._consider(target, record);
}
...
export function notifyCharacterData(target: unknown, oldValue: string): void {
	if (allObservers.length === 0) return;
	const record: MutationRecord = {
		type: 'characterData',
		target,
