...
//      `sdmc:/` rather than `browser://`. Three.js's WebGLTextures
...
try {
	...
	// recognise `browser://`, so every Image src has to point at sdmc:/.
	// Path matches the romfs install + the Citron-profile mirror.
	const ASSET_PREFIX_SDMC =
		'sdmc:/switch/webprofiles/default/pages/threejs-demos/webgl-materials-cubemap/assets/';
	const ASSET_PREFIX_BROWSER =
		'browser://threejs-demos/webgl-materials-cubemap/assets/';
	...
	function init() {
		...
		// `this.path`), so the full browser:// URL goes in.
