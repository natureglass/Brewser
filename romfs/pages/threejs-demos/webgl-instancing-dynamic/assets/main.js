...
//      runtime doesn't define. URL goes through `browser://`.
...
try {
	...
	function init() {
		...
		// satisfy. URL through browser:// (sdmc:/ direct fetch is 403'd by
		// the WebView's `enableLocalFetch: false`).
		(async function loadModel() {
			try {
				const resp = await fetch(
					'browser://threejs-demos/webgl-instancing-dynamic/assets/suzanne_buffergeometry.json'
				);
