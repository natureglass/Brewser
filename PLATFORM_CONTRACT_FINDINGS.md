...
- `versions.json` — **MOVED to `brewser-apps-staging`** (served at `my.brewser.tech/versions.json`). Its sole producer is now `brewser-v8`'s `make release` mirror of `romfs/configs/current.json`; the old `scripts/collect_versions.py` generator (and the `brewser-apps` Makefile `versions` target) was retired.
