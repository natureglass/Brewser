...
**Runtime (stale):** `brewser-v8` (Switch shell), `brewser-runtime-v8` (engine library).
...
categories.json  tags.json  permissions.json  versions.json
...
- `versions.json` = `{ "nx.js": "0.0.70", "runtime": "0.0.1", "brewser": "0.1.1" }` (`versions.json:1-5`), generated by `scripts/collect_versions.py:23-48` from three *external* workspace package.json files.
- `categories.json` (18 **genre** labels — despite the filename it matches the manifest `genre` vocabulary, not `categories`), `tags.json` (4 hardware tags), `permissions.json` (8 lowercase keys `network, storage, system, filesystem_read, filesystem_write, device_info, account, external_links` with `{show,risk,description,warning}`) — **hand-maintained, generated by nothing, consumed by nothing in this repo** (single git touch at the reset commit). `permissions.json` has **no `usb` entry** and its keys do not match the manifest permission strings.
...
### 4.1 Shell (`brewser-v8`) — where the platform is actually spoken

The platform URLs physically ship in the **vendored compiled runtime** `runtime/dist/runtime-defaults.js` (the `@switch-web/runtime` symlink targets `brewser-v8/runtime`, **not** `brewser-runtime-v8`):
...
✔ The engine **source** already repointed downloads/ratings to WP (`brewser-runtime-v8/src/runtime-defaults.ts:82-83`) — the shell's vendored dist predates that change. This is the only substantive difference between the two copies.
...
**Versions check:** fetch prod `versions.json`, compare string-inequality per key against seeded `configs/current.json` (`updates-modal.js:374-455`). Prod file has exactly the three expected keys — **compatible**. (`config.json:18` `checkVerOnBoot` is an orphan — parsed nowhere.)
...
|---|---|---|
| Catalogue envelope | `{version:1, generated, featured:[], experimental:[], community:[]}` (✔ `brewser-v8/romfs/configs/catalogue.json:1-4`) | `{version:1, generated, apps:[]}` (✔ `brewser-apps/catalogue.json:1-4`) |
...
- Live GitHub fine-grained PAT committed at `brewser-runtime-v8/telemetry/Test-GitHubDispatch.ps1:38` (targets the retired telemetry repo). Should be revoked regardless of adaptation choice.
