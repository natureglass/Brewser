...
import { resolveLiveResourceUrl } from './live-dom.js';
function getVideoDecoderCtor() {
    const sw = globalThis.Switch;
    if (!sw || typeof sw.VideoDecoder !== 'function')
        return null;
...
export function setVideoTryHwAccel(enabled) {
...
const elementsWithPosters = new Set();
...
// =========================================================================
function resolveSourceForDecoder(el) {
...
// Decoder lifecycle
// =========================================================================
function openDecoder(el, st, wantHw, opts = {}) {
...
export function paintVideoFrameAt(ctx, el, dstX, dstY, dstW, dstH) {
    ...
    if (!useBitmap && !st.hasFirstFrame)
        return false;
    if (st.frameW <= 0 || st.frameH <= 0)
        return false;
    ...
    if (useBitmap) {
        try {
            ctx.drawImage(useBitmap, 0, 0, st.frameW, st.frameH, innerX, innerY, innerW, innerH);
            return true;
        }
        catch {
...
export function tickVideo() {
    ...
    for (const el of activeVideos) {
        ...
        if (hwError || hwStall || hwFrameStall) {
            if (hwStall || hwFrameStall)
                gHwStallDetected = true;
            if (!hwFallback)
                hwFallback = [];
            hwFallback.push(el);
            continue;
        ...
        try {
            ...
            if (bridge?.createBitmapFromRGBA && bridge.writeRGBAToBitmap) {
                ...
                const recreateThisFrame = !VIDEO_BITMAP_REUSE_BETWEEN_FRAMES || dimsChanged;
                if (recreateThisFrame) {
                    ...
                    try {
                        st.frameBitmap = bridge.createBitmapFromRGBA(f.data, f.width, f.height);
                    }
                    catch {
                        st.frameBitmap = null;
                    }
                }
                else if (st.frameBitmap) {
                    try {
    ...
    if (hwFallback) {
        ...
        if (gHwStallDetected) {
            const cascade = [];
            for (const el of activeVideos) {
                if (hwFallback.includes(el))
                    continue;
                const st = videoStateMap.get(el);
                if (!st || !st.decoder || st.hwFallbackAttempted)
                    continue;
                let usedHw = false;
                try {
                    usedHw = st.decoder.usedHw === true;
                }
                catch { /* ignore */ }
                if (usedHw)
                    cascade.push(el);
...
export function pageHasActiveVideo() {
...
export function pageHasAnyPoster() {
...
export function scanForAutoplayVideos(root) {
    const visit = (el) => {
        if (el.tagName === 'VIDEO') {
            ...
            if (wantAutoplay) {
                ...
                if (!st.decoder && !st.loadError && !st.posterBitmap) {
                    ...
                    openDecoder(el, st, /* wantHw */ false, { noAudio: true, silentOnError: true });
                    const dec = st.decoder;
                    if (!dec) {
...
// =========================================================================
export function videoIsPaused(el) {
    const st = videoStateMap.get(el);
    if (!st || !st.decoder)
        return true;
    try {
        return st.decoder.paused;
    }
    catch {
        return true;
    }
}
export function videoIsEnded(el) {
    const st = videoStateMap.get(el);
    if (!st || !st.decoder)
        return false;
    try {
        return st.decoder.ended;
    }
    catch {
        return false;
    }
}
export function videoCurrentTime(el) {
    const st = videoStateMap.get(el);
    if (!st)
        return 0;
...
export function videoGetAudioLevels(el) {
    const st = videoStateMap.get(el);
    if (!st || !st.decoder)
        return [];
    try {
 ...
 * music visualizer. */
export function videoGetFrequencyData(el, out) {
    const st = videoStateMap.get(el);
    if (!st || !st.decoder)
        return false;
    try {
        return st.decoder.getFrequencyData(out) === true;
    }
    catch {
        return false;
    }
...
export function videoGetWaveform(el, out) {
    const st = videoStateMap.get(el);
    if (!st || !st.decoder)
        return false;
    try {
        return st.decoder.getWaveform(out) === true;
    }
    catch {
        return false;
    }
 ...
 * re-opens. */
export function videoGetVolume(el) {
    const st = videoStateMap.get(el);
    return st ? st.volume : 1;
}
export function videoSetVolume(el, v) {
    const st = ensureState(el);
    st.volume = Number.isFinite(v) ? Math.max(0, Math.min(1, v)) : 1;
    if (st.decoder) {
        try {
            st.decoder.setVolume(st.volume);
        }
        catch { /* ignore */ }
    }
}
export function videoIsMuted(el) {
    const st = videoStateMap.get(el);
    return st ? st.muted : false;
}
export function videoSetMuted(el, m) {
    const st = ensureState(el);
    st.muted = !!m;
    if (st.decoder) {
        try {
            st.decoder.setMuted(st.muted);
        }
        catch { /* ignore */ }
    }
}
export function videoDuration(el) {
    const st = videoStateMap.get(el);
    if (!st || !st.decoder)
        return 0;
    try {
        return st.decoder.duration;
    }
    catch {
        return 0;
    }
}
export function videoPlay(el) {
    const st = ensureState(el);
    ...
    if (dec) {
        ...
        try {
            dec.setVolume(st.volume);
        }
        catch { /* ignore */ }
        try {
            dec.setMuted(st.muted);
        }
        catch { /* ignore */ }
        try {
            dec.play();
...
export function videoSeek(el, t) {
    const st = videoStateMap.get(el);
    if (st && st.decoder) {
        try {
            st.decoder.seek(t);
...
export function videoResetSource(el) {
    const st = videoStateMap.get(el);
    if (!st)
        return;
    closeDecoder(el, st);
...
export function videoToggleMute(el) {
    const st = videoStateMap.get(el);
    if (!st || !st.decoder)
        return;
    try {
...
export function videoSeekRatio(el, ratio) {
    const st = videoStateMap.get(el);
    if (!st || !st.decoder)
        return;
...
export function hitTestVideoControls(boxX, boxY, boxW, boxH, tapX, tapY, el) {
    ...
    if (el) {
        ...
        if (st && st.decoder) {
            try {
                audioOnly = st.decoder.usedVideo === false;
            }
            catch { /* ignore */ }
        }
    }
...
export function paintVideoControls(ctx, el, boxX, boxY, boxW, boxH) {
    if (boxW <= 0 || boxH <= 0)
        return;
    ...
    if (st && st.decoder) {
        try {
            isPaused = st.decoder.paused;
        }
        catch { /* ignore */ }
        try {
            isEnded = st.decoder.ended;
        }
        catch { /* ignore */ }
        try {
            muted = st.decoder.muted;
...
export function videoErrorMessage(el) {
    const st = videoStateMap.get(el);
    if (!st)
        return null;
    if (st.loadError)
