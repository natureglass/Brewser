...
import { getComputedLiveStyle, getKeyframes, someStylesheetUsesSibling } from './live-css.js';
import { ensureCssAnimation, getBackgroundImage, getCssAnimState, getLiveTreeVersion, isEngineModalRoot } from './live-dom.js';
import { buildFormSubmitUrl, findEnclosingForm, paintFormWidget } from './live-form.js';
import { VIDEO_CONTROLS_BAR_H, hitTestVideoControls, paintVideoControls, paintVideoFrameAt, } from './live-video.js';
import { collectAbsolutes, findAbsoluteContainingBlock, getFieldsetLegendCutout, getInlineLayout, getLayoutBox, layoutAbsoluteRoot, layoutFixedRoot, resetLayoutCache, setLayoutMeasureCtx, } from './live-layout.js';
...
export function getLiveContentBottom() {
...
export function patchLiveCacheRegion(el) {
    ...
    try {
        ...
        const rect = pixelAlignRect({ x: box.x, y: box.y, w: box.w, h: box.h });
        cacheCtx.beginPath();
        ...
        if (root) {
            ...
            if (bodyBox)
                paintBoxedElement(cacheCtx, root, bodyCs, bodyBox);
            const ops = [];
            ...
            for (const op of ops) {
                ...
                const b = op.kind === 'atom' ? op.atom : op.box;
                if (!b || !rectsIntersect(b, rect))
                    continue;
                try {
...
export function patchLiveCacheRegions(els) {
    if (els.length === 0 || !liveCacheOffscreen)
        return;
    const cacheCtx = liveCacheOffscreen.getContext('2d');
    if (!cacheCtx)
        return;
    ...
    try {
        ...
        for (const r of rects)
            cacheCtx.rect(r.x, r.y, r.w, r.h);
        cacheCtx.clip();
        for (const r of rects)
            cacheCtx.clearRect(r.x, r.y, r.w, r.h);
...
export function patchLiveImagePixelsOnly(els) {
    if (els.length === 0 || !liveCacheOffscreen)
        return;
    const cacheCtx = liveCacheOffscreen.getContext('2d');
    if (!cacheCtx)
    ...
    try {
        ...
        for (const el of els) {
            ...
            try {
                paintBoxedElement(cacheCtx, el, cs, box);
            }
...
export function syncLiveCacheVersion() {
 ...
 * same path. */
function isTranslucentColorValue(bg) {
    if (!bg)
        return false;
    ...
    if (rgbaM) {
        if (rgbaM[1] === undefined)
            return false; // rgb() w/o alpha → opaque
        const a = parseFloat(rgbaM[1]);
        return Number.isFinite(a) && a < 1;
    }
    const hslaM = /^hsla?\(\s*[^,]+,\s*[^,]+,\s*[^,]+(?:,\s*([0-9.]+))?\s*\)$/.exec(t);
    if (hslaM) {
        if (hslaM[1] === undefined)
            return false;
...
function _partialDbg(msg) {
    if (!PARTIAL_DEBUG || _partialDbgCount >= 400)
        return;
    _partialDbgCount++;
    try {
...
export function patchLiveDirtyRegions() {
    ...
    const root = lastPaintedRoot;
    if (!root) {
        _partialDbg('  PUNT: no-root → full rebuild');
        return false;
    }
    const vpW = lastBodyViewportW;
    const vpH = lastBodyViewportH;
    if (vpW <= 0 || vpH <= 0) {
        _partialDbg('  PUNT: bad-viewport ' + vpW + 'x' + vpH);
        return false;
    }
    const cacheCtx = liveCacheOffscreen.getContext('2d');
    if (!cacheCtx) {
        _partialDbg('  PUNT: no-ctx');
        return false;
    }
    ...
    for (const el of dirty) {
        ...
        if (pos === 'absolute') {
            ...
            if (!a && !bAdded) {
                _partialDbg('  skip no-box abs ' + _tagOf(el));
            }
            else
                _partialDbg('  ' + _tagOf(el) + ' abs-relaid');
            continue;
        }
    ...
    for (let i = 0; i < rects.length; i++)
        rects[i] = pixelAlignRect(rects[i]);
...
export function setLiveBuildChunkMs(ms) {
 ...
 * `setLiveBuildChunkMs`. */
export function setLiveScrollChunkMs(ms) {
    if (Number.isFinite(ms) && ms > 0)
        scrollChunkMs = ms;
}
...
let scrollOverlayEls = [];
const scrollContentCaches = new WeakMap();
...
const scrollContentVersions = new WeakMap();
function currentScrollContentVersion(el) {
...
function noteScrollDirty(dirtyEls) {
    if (dirtyEls.length === 0 || scrollOverlayEls.length === 0)
        return;
...
export function isLiveCacheBuilding() { return cacheBuilding; }
...
export function isLiveCacheReady() {
...
export function resetLiveOverlayCache() {
    lastBodyVersion = -1;
    lastBodyViewportW = -1;
    lastBodyViewportH = -1;
...
export function paintLiveOverlay(ctx, root, viewport = { x: 0, y: 0, width: 0, height: 0 }, scrollY = 0, options = {}) {
    ...
    if (options.skipFlow) {
        ...
        if (dirty) {
            ...
            lastLiveContentBottom = Math.max(bodyBox.h, bodyBox.contentY + bodyBox.intrinsicContentH);
            // (Re)allocate the cache OffscreenCanvas. Cleared once at
            // build start; chunked ops paint into it over multiple
            // frame yields.
            const cacheH = Math.max(viewport.height, Math.min(lastLiveContentBottom, LIVE_CACHE_MAX_H));
            if (!liveCacheOffscreen
        ...
        if (liveCacheOffscreen) {
            ...
            if (visibleH > 0) {
                ...
                try {
                    ...
                    ctx.clip();
                    ctx.drawImage(liveCacheOffscreen, 0, srcY, viewport.width, visibleH, viewport.x, viewport.y, viewport.width, visibleH);
                }
                finally {
                    ctx.restore();
                }
            ...
            if (wipeBg) {
                ...
                try {
                    ...
                    for (const r of wipeRects) {
                        ...
                        if (sBottom <= cacheScreenBottom)
                            continue; // fully covered by cache blit
        ...
        for (const sEl of scrollOverlayEls) {
                        ...
                        if (inline) {
                            for (const atom of inline.atoms) {
                                if (atom.isBr)
                                    continue;
                                try {
                                    paintOneInlineAtom(sctx, atom);
                                }
                                catch (_) { /* skip a bad atom */ }
                            }
                        }
                        for (const c of sEl.children)
                            paintSubtreeLaid(sctx, c);
                        void sCs;
                    }
                    finally {
                        sctx.restore();
            ...
            try {
                ...
                if (visibleH > 0) {
                    ctx.drawImage(sc.canvas, 0, st, cw, visibleH, sBox.contentX, sBox.contentY, cw, visibleH);
                }
...
// =========================================================================
let kbCacheOffscreen = null;
let kbCacheW = 0;
let kbCacheH = 0;
let kbCacheRoot = null;
...
export function paintKeyboardOverlay(ctx, root, viewport) {
    ...
    const needsBuild = !kbCacheOffscreen
        || kbCacheW !== w
        || kbCacheH !== h
        || kbCacheRoot !== root
    ...
    if (kbCacheOffscreen) {
        ...
        try {
            ...
            ctx.clip();
            ctx.drawImage(kbCacheOffscreen, 0, 0, w, h, viewport.x, viewport.y, w, h);
        }
        finally {
            ctx.restore();
        }
 ...
 * routine cases. */
export function resetKeyboardOverlayCache() {
...
// viewport.
// =========================================================================
let toolbarCacheOffscreen = null;
let toolbarCacheW = 0;
let toolbarCacheH = 0;
let toolbarCacheRoot = null;
...
export function paintToolbarOverlay(ctx, root, viewport) {
    const w = Math.max(1, Math.floor(viewport.width));
    const h = Math.max(1, Math.floor(viewport.height));
    const version = getLiveTreeVersion() + getToolbarTreeVersion();
    const needsBuild = !toolbarCacheOffscreen
        || toolbarCacheW !== w
        || toolbarCacheH !== h
    ...
    if (toolbarCacheOffscreen) {
        ...
        try {
            ...
            ctx.rect(viewport.x, viewport.y, w, h);
            ctx.clip();
            ctx.drawImage(toolbarCacheOffscreen, 0, 0, w, h, viewport.x, viewport.y, w, h);
        }
        finally {
            ctx.restore();
        }
 ...
 * routine cases. */
export function resetToolbarOverlayCache() {
 ...
 * paint work. */
export function paintModalOverlay(ctx, viewport) {
    if (!hasAnyModalRoot())
        return;
    const w = Math.max(1, Math.floor(viewport.width));
    const h = Math.max(1, Math.floor(viewport.height));
    ...
    for (const root of getModalRoots()) {
        ...
        if (!rootBox || rootBox.w <= 0 || rootBox.h <= 0)
            continue;
        let cache = modalCaches.get(root);
        const needsBuild = !cache
            || cache.w !== w
            || cache.h !== h
            || cache.version !== version;
        let active = cache ?? null;
        if (needsBuild) {
        ...
        try {
            ...
            ctx.rect(viewport.x, viewport.y, w, h);
            ctx.clip();
            ctx.drawImage(active.off, 0, 0, w, h, viewport.x, viewport.y, w, h);
        }
        finally {
            ctx.restore();
        }
...
export function resetModalOverlayCache() {
...
export function overlayLiveAnimatedCanvases(ctx, root, viewport, scrollY, copyBridgeToScreen) {
    ctx.save();
    try {
        ctx.beginPath();
        ctx.rect(viewport.x, viewport.y, viewport.width, viewport.height);
        ...
        const overlayVerbose = globalThis
        ...
        const visit = (el, inFixed) => {
            ...
            if (el.tagName === 'CANVAS' && !nowFixed) {
                ...
                if (box && box.w > 0 && box.h > 0) {
                    ...
                    if (visible) {
                        if (webglBacked) {
                            copyBridgeToScreen(0, 0, box.w, box.h, screenX, screenY);
                        }
                        else {
                            const off = el.getOffscreen();
                            if (off) {
                                try {
                                    ctx.drawImage(off, screenX, screenY, box.w, box.h);
                                }
                                catch (_) { /* swallow */ }
                            }
                        }
                ...
                if (box && box.w > 0 && box.h > 0) {
                    ...
                    if (screenY + box.h >= vTop && screenY <= vBot) {
                        ...
                        if (!havePainted && hasControls) {
                            ...
                            ctx.fillRect(screenX, screenY + box.h - barH, box.w, barH);
                        }
                        if (hasControls) {
...
export function findTapIntent(target, tapX = 0, tapY = 0, viewportX = 0, viewportY = 0, scrollY = 0) {
    for (let n = target; n; n = n.parent) {
        ...
        if (dataAction)
            return { kind: 'button-action', action: dataAction };
        ...
        if (dblAction)
            return { kind: 'dbltap-action', action: dblAction, el: n };
        if (n.tagName === 'A') {
        ...
        if (n.tagName === 'VIDEO') {
            ...
            if (box) {
                ...
                const ctrl = hitTestVideoControls(screenX, screenY, box.w, box.h, tapX, tapY, n);
                if (ctrl)
                    return { kind: 'video-control', control: ctrl, video: n };
            }
...
function paintSubtreeLaid(ctx, el) {
    const cs = getComputedLiveStyle(el);
    if (cs.display === 'none')
        return;
...
function paintSubtreeRest(ctx, el, cs, box) {
    ...
    if (clips || scrolls) {
        ...
        if (scrolls)
            paintLiveScrollbarV(ctx, box, el.scrollTop);
        return;
    }
    for (const c of el.children)
        paintSubtreeLaid(ctx, c);
...
function paintLiveScrollbarV(ctx, box, scrollTop) {
    const maxScroll = Math.max(0, box.intrinsicContentH - box.contentH);
    if (maxScroll <= 0 || box.contentH <= 0)
        return;
    const trackW = 6;
    const inset = 3;
...
function beginCssAnimationXform(ctx, el, cs, box) {
...
function paintBoxedElement(ctx, el, cs, box) {
...
function paintBoxedElementInner(ctx, el, cs, box) {
    const tag = el.tagName;
            ...
            try {
                if (radius > 0) {
                    ...
                    try {
                        ...
                        ctx.clip();
                        ctx.drawImage(off, box.x, box.y, w, h);
                    }
                    finally {
                        ctx.restore();
                    }
    ...
    if (isFormTag) {
        ...
        if (paintFormWidget(ctx, el, cs, box, richBg))
            return;
    }
    if (box.w > 0 && box.h > 0) {
        paintOuterBoxShadows(ctx, cs, box, radius);
    ...
    if (cs.beforeStyle || cs.afterStyle) {
        if (box.w > 0 && box.h > 0) {
            if (cs.beforeStyle)
                paintPseudoBox(ctx, cs.beforeStyle, box, radius);
            if (cs.afterStyle)
                paintPseudoBox(ctx, cs.afterStyle, box, radius);
        }
    }
...
function paintInlineAtoms(ctx, layout) {
...
function paintOneInlineAtom(ctx, atom) {
    if (atom.isBr)
        return; // pure layout marker
    ...
    if (atom.emojiKey) {
        ...
        if (img) {
            try {
                ctx.drawImage(img, atom.x, atom.y, atom.w, atom.h);
            }
        ...
        if (!emojiAssetFailed(atom.emojiKey))
            return;
    ...
    if (atom.el.tagName === 'CANVAS') {
        ...
        if (off) {
            try {
                ctx.drawImage(off, atom.x, atom.y, atom.w, atom.h);
            }
            catch (_) { /* swallow — drawImage on uninitialised canvas */ }
        }
        return;
    ...
    try {
        ...
        ctx.textAlign = 'left';
        if (italic)
            ctx.transform(1, 0, -0.2, 1, drawY * 0.2, 0);
        ...
        if (drawn === 0) {
            ...
            if (bold)
                ctx.fillText(atom.text, atom.x + 1, drawY);
        }
        const td = cs.textDecoration;
        if (td && td !== 'none' && !atom.text.match(/^\s*$/)) {
            let lineY;
            if (td === 'underline')
                lineY = drawY + fontSize * 0.45;
            else if (td === 'overline')
                lineY = drawY - fontSize * 0.5;
            else
                lineY = drawY;
            ctx.fillStyle = color;
...
function collectPaintOps(el, out, skipBgOfRoot, scrollOut) {
    const cs = getComputedLiveStyle(el);
    if (cs.display === 'none')
        return;
    ...
    if (opacity <= 0)
        return;
    if (el.style.position === 'fixed' && !skipBgOfRoot)
        return;
    ...
    if (!skipBgOfRoot && isEngineModalRoot(el))
        return;
    const box = getLayoutBox(el);
    if (!box) {
        // Table sectioning elements (THEAD / TBODY / TFOOT / TR) don't get
...
function paintListMarker(ctx, el, cs, box) {
    ...
    if (styleType === 'decimal')
        ...
        text = String.fromCharCode(96 + ((index - 1) % 26) + 1) + '.';
    else if (styleType === 'upper-alpha')
        text = String.fromCharCode(64 + ((index - 1) % 26) + 1) + '.';
    else if (styleType === 'lower-roman')
        text = toRoman(index).toLowerCase() + '.';
    else if (styleType === 'upper-roman')
        text = toRoman(index) + '.';
    else
        return;
    ctx.save();
...
function paintIframe(ctx, el, box) {
    if (box.w <= 0 || box.h <= 0)
        return;
    ctx.save();
    try {
        ...
        if (!hasContent) {
            ...
            const failed = el._iframeLoadFailed === true;
...
function paintImg(ctx, el, cs, box, radius = 0) {
...
function paintVideoPlaceholder(ctx, el, cs, box, radius = 0) {
...
function resolveVideoSourceUrl(el) {
...
function paintLiveSvg(ctx, el, box) {
...
function paintBar(ctx, box, trackBg, fillColor, fillFrac) {
 ...
 * track background. */
function paintMeter(ctx, el, cs, box) {
...
function paintProgress(ctx, el, cs, box) {
 ...
 * fillStyle. */
function paintIconGlyph(ctx, text, x, y, fontSize) {
    if (text.length !== 1)
        return 0;
    switch (text) {
        case '▾': {
 ...
 */
function paintBorders(ctx, cs, box, radius = 0, topCutout) {
    if (box.w <= 0 || box.h <= 0)
        return;
    ...
    if (tw > 0 && tc && tc !== 'transparent') {
        ...
        if (topCutout && topCutout.w > 0) {
            ...
            if (leftW > 0)
                ctx.fillRect(box.x, box.y, leftW, tw);
            if (rightW > 0)
                ctx.fillRect(rightX, box.y, rightW, tw);
        }
        else {
...
function resolveLiveBorderRadius(cs, w, h) {
    const v = cs.borderRadius;
    if (!v || w <= 0 || h <= 0)
        return 0;
    const maxR = Math.min(w, h) / 2;
    if ('px' in v)
        return Math.max(0, Math.min(v.px, maxR));
 ...
 * the path. */
function paintOuterBoxShadows(ctx, cs, box, radius) {
    ...
    for (const sh of shadows) {
        ...
        try {
            ...
            if (sr > 0)
                pathLiveRoundedRect(ctx, sx, sy, sw, sh_, sr);
            else {
                ctx.beginPath();
                ctx.rect(sx, sy, sw, sh_);
            }
            ctx.fill();
        }
        finally {
 ...
 * away. */
function paintInsetBoxShadows(ctx, cs, box, radius) {
    const shadows = cs.boxShadow;
    if (!shadows || shadows.length === 0)
        return;
    for (const sh of shadows) {
...
function pathLiveRoundedRectReverse(ctx, x, y, w, h, r) {
...
function paintBackground(ctx, cs, box, radius) {
            ...
            for (const layer of layers) {
                ...
                if (layer.type === 'image') {
                    const effLayer = (size && layer.sizeMode === undefined
                        ...
                        ? { ...layer, sizeMode: 'auto', sizeW: size.w, sizeH: size.h }
                        : layer;
                    paintBackgroundLayer(ctx, effLayer, box.x, box.y, box.w, box.h);
                    continue;
                }
                if (size)
                    paintTiledLayer(ctx, layer, box.x, box.y, box.w, box.h, size.w, size.h);
                else
...
function paintBackgroundLayer(ctx, layer, x, y, w, h) {
    if (layer.type === 'solid') {
    ...
    if (layer.type === 'image') {
        ...
        if (!img)
            return; // not loaded (or failed) — leave layer transparent
        ...
        if (layer.sizeMode === 'cover') {
            ...
            // width-from-aspect.
            if (layer.sizeW && layer.sizeH) {
                dw = layer.sizeW;
                dh = layer.sizeH;
            }
            else if (layer.sizeW) {
                dw = layer.sizeW;
                dh = natH * (layer.sizeW / natW);
            }
            else if (layer.sizeH) {
                dh = layer.sizeH;
                dw = natW * (layer.sizeH / natH);
            }
            else {
                dw = natW;
                dh = natH;
...
function paintTiledLayer(ctx, layer, x, y, w, h, tw, th) {
    if (tw <= 0 || th <= 0) {
        paintBackgroundLayer(ctx, layer, x, y, w, h);
        return;
    }
...
function paintPseudoBox(ctx, pseudo, hostBox, hostRadius) {
    if (!pseudo)
        return;
    if (!pseudo.backgroundLayers && !pseudo.background)
        return;
    ...
    try {
        ...
        if (pseudo.maskImage)
            paintPseudoMaskedBg(ctx, pseudo, px, py, pw, ph);
        else
            paintPseudoBgLayers(ctx, pseudo, px, py, pw, ph);
    }
    finally {
        ctx.restore();
    }
...
function paintPseudoBgLayers(ctx, pseudo, x, y, w, h) {
    ctx.save();
    try {
        ctx.beginPath();
        ctx.rect(x, y, w, h);
        ...
        if (layers && layers.length > 0) {
            ...
            for (const layer of layers) {
                if (size)
                    paintTiledLayer(ctx, layer, x, y, w, h, size.w, size.h);
                else
                    paintBackgroundLayer(ctx, layer, x, y, w, h);
            }
        }
...
function paintPseudoMaskedBg(ctx, pseudo, x, y, w, h) {
    ...
    try {
        ...
        if (!octx) {
            paintPseudoBgLayers(ctx, pseudo, x, y, w, h);
            return;
        }
...
function applyStops(grad, stops, extentPx = 0) {
...
function normalizeGradientColor(color) {
    const t = color.trim().toLowerCase();
    if (t === 'transparent')
        return 'rgba(0,0,0,0)';
    return color;
}
function pathLiveRoundedRect(ctx, x, y, w, h, r) {
    const cr = Math.min(r, w / 2, h / 2);
    ctx.beginPath();
    ctx.moveTo(x + cr, y);
    ctx.lineTo(x + w - cr, y);
 ...
 */
function paintLiveText(ctx, el, cs, originX, originY, w, h, hostBox) {
    const ownText = el.textContent;
    const beforeText = cs.before;
    const afterText = cs.after;
    if (!ownText && !beforeText && !afterText)
        return;
    ...
    try {
        ...
        if (beforeText) {
            ...
            if (iconW > 0) {
                ...
                if (bold)
                    ctx.fillText(beforeText, originX + 3 + chevronAdvance, drawY);
                beforeAdvance = chevronAdvance + 2 + ctx.measureText(beforeText).width + 4;
            }
            paintTextDecorationLine(ctx, cs, beforeText, originX + 2 + chevronAdvance, drawY, fontSize, color);
        }
        ...
        if (ownText) {
            ...
            if (w > 0) {
                if (align === 'center')
                    textX = originX + (w + beforeAdvance) / 2;
            ...
            if (bold)
                ctx.fillText(ownText, textX + 1, drawY);
            paintTextDecorationLine(ctx, cs, ownText, textX, drawY, fontSize, color, align);
        }
        ...
        if (afterText) {
            ...
            // padding).
            paintPseudoText(ctx, afterText, cs.afterStyle, originX, originY, w, h, drawY, fontSize, color, bold, 'right', hostBox);
        }
    }
    finally {
        ctx.restore();
...
function paintPseudoText(ctx, text, pseudo, originX, originY, w, h, drawY, hostFontSize, hostColor, hostBold, defaultAlign, hostBox) {
    ...
    if (isAbs) {
        ...
        if (hostBold)
            ctx.fillText(text, ax + 1, drawY);
    }
    else {
        ctx.textAlign = 'left';
        ctx.fillText(text, originX + 2, drawY);
        if (hostBold)
            ctx.fillText(text, originX + 3, drawY);
    }
    ctx.fillStyle = prevFillStyle;
    if (restoreFont !== undefined)
...
function paintTextDecorationLine(ctx, cs, text, x, textY, fontSize, color, align = 'left') {
    const td = cs.textDecoration;
    if (!td || td === 'none')
        return;
    ...
    let lineY;
    if (td === 'underline')
        lineY = textY + fontSize * 0.45;
    else if (td === 'overline')
        lineY = textY - fontSize * 0.5;
    else
        lineY = textY; // line-through at middle
    ctx.save();
    try {
        ctx.fillStyle = color;
 ...
 * offset line. */
function paintTextDecorationStyledRun(ctx, x, y, w, style) {
    if (w <= 0)
        return;
    if (style === 'dotted') {
...
export function hasLiveOverlay(root) {
...
//# sourceMappingURL=live-overlay.js.map