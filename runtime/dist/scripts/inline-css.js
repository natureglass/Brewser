...
export function isPercent(v) {
    return typeof v === 'object' && v !== null && typeof v.percent === 'number';
}
...
export function resolveLength(v, basis) {
    if (v === undefined)
        return undefined;
    if (typeof v === 'number')
        return v;
...
export function applyDecl(style, propRaw, valueRaw) {
    ...
    if (prop === 'border-radius') {
        ...
        if (pct) {
            ...
            if (Number.isFinite(n))
                style.borderRadius = { percent: n / 100 };
            return;
        }
        const n = parsePxOrNum(first);
        if (n !== undefined)
            style.borderRadius = { px: n };
        return;
    }
...
function parseOverflowValue(v) {
    const t = v.trim().toLowerCase();
    if (t === 'visible' || t === 'hidden' || t === 'scroll' || t === 'auto')
        return t;
    return undefined;
}
...
function applyShorthand(style, prop, value) {
 ...
 */
function applyBorderShorthand(style, prop, value, tokens) {
    ...
    const setSide = (side) => {
        style['border' + side + 'Width'] = widthVal;
        if (color !== undefined)
...
function expandBoxShorthand(values) {
 ...
 */
export function resolveCanvasFont(style) {
...
export function quoteFontFamily(family) {
    ...
    if (lastLower !== 'sans-serif')
        out.push('sans-serif');
    return out.join(', ');
}
/** True iff the resolved weight is bold (numeric ≥600 or 'bold'). */
export function isBoldWeight(style) {
...
export function isItalicStyle(style) {
...
export function serializeStyle(style) {
    ...
    if (style.top !== undefined)
        parts.push('top:' + style.top + 'px');
    if (style.left !== undefined)
        parts.push('left:' + style.left + 'px');
    if (style.right !== undefined)
        parts.push('right:' + style.right + 'px');
    if (style.bottom !== undefined)
        parts.push('bottom:' + style.bottom + 'px');
    if (style.width !== undefined)
        parts.push('width:' + serializeLen(style.width));
    if (style.height !== undefined)
        parts.push('height:' + serializeLen(style.height));
    if (style.display !== undefined)
        parts.push('display:' + style.display);
    if (style.opacity !== undefined)
        parts.push('opacity:' + style.opacity);
    if (style.zIndex !== undefined)
        parts.push('z-index:' + style.zIndex);
    if (style.cursor !== undefined)
        parts.push('cursor:' + style.cursor);
    ...
    if (style.listStyleType !== undefined)
        parts.push('list-style-type:' + style.listStyleType);
    ...
    if (style.marginRight !== undefined)
        parts.push('margin-right:' + style.marginRight + 'px');
    if (style.marginBottom !== undefined)
        parts.push('margin-bottom:' + style.marginBottom + 'px');
    if (style.marginLeft !== undefined)
        parts.push('margin-left:' + style.marginLeft + 'px');
    ...
    if (style.flexBasis !== undefined)
        parts.push('flex-basis:' + style.flexBasis + 'px');
    ...
    if (style.boxSizing !== undefined)
        parts.push('box-sizing:' + style.boxSizing);
    if (style.minWidth !== undefined)
        parts.push('min-width:' + serializeLen(style.minWidth));
    if (style.maxWidth !== undefined)
        parts.push('max-width:' + serializeLen(style.maxWidth));
    if (style.minHeight !== undefined)
        parts.push('min-height:' + serializeLen(style.minHeight));
    if (style.maxHeight !== undefined)
        parts.push('max-height:' + serializeLen(style.maxHeight));
    if (style.overflowX !== undefined)
        parts.push('overflow-x:' + style.overflowX);
    if (style.overflowY !== undefined)
        parts.push('overflow-y:' + style.overflowY);
    return parts.join(';');
}
...
let cssVpW = 1280;
let cssVpH = 720;
export function setCssViewport(w, h) {
    cssVpW = w;
    cssVpH = h;
}
 ...
 * content viewport. */
export function getCssViewport() {
    return { w: cssVpW, h: cssVpH };
}
function parsePxOrNum(s) {
    const t = s.trim();
    ...
    if (vh)
        return parseFloat(vh[1]) * 0.01 * cssVpH;
    const vw = /^(-?\d+(?:\.\d+)?)vw$/.exec(t);
    if (vw)
        return parseFloat(vw[1]) * 0.01 * cssVpW;
...
export function resolveFontSizeKeyword(value) {
...
export function parseLength(s) {
    ...
    if (fn) {
        ...
        if (args.length === 0)
            return undefined;
        return { fn: fn[1].toLowerCase(), args };
    }
    if (t.endsWith('%')) {
...
function serializeLen(v) {
    if (typeof v === 'number')
        return v + 'px';
    if ('fn' in v)
        return v.fn + '(' + v.args.map(serializeLen).join(', ') + ')';
    return v.percent + '%';
}
function clamp01(n) {
    if (n < 0)
 ...
 * `style['z-index']`. */
export function cssToJsProp(name) {
    if (name.indexOf('-') < 0)
        return name;
    return name.replace(/-([a-z])/g, (_m, c) => c.toUpperCase());
}
export function jsToCssProp(name) {
