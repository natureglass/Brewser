...
import { getCssViewport, parseLength, resolveFontSizeKeyword } from './inline-css.js';
...
const focusElements = new WeakSet();
...
const hoverElements = new WeakSet();
 ...
 */
const stylesheetsWithActive = new Set();
const stylesheetsWithFocus = new Set();
const stylesheetsWithChecked = new Set();
const stylesheetsWithHover = new Set();
 ...
 * on tab UIs are rare. */
const stylesheetsWithSibling = new Set();
function rulesUsePseudo(rules, kind) {
 ...
 * required. */
function rulesUseSibling(rules) {
    for (const rule of rules) {
...
function chainUsesPseudo(chain, kind) {
...
function someActiveRuleAffectsElement(el) {
    for (const rules of styleSheets.values()) {
        for (const rule of rules) {
            if (!chainUsesPseudo(rule.chain, 'active'))
                continue;
...
function someFocusRuleAffectsElement(el) {
    for (const rules of styleSheets.values()) {
        for (const rule of rules) {
            if (!chainUsesPseudo(rule.chain, 'focus'))
                continue;
...
export function reserveStylesheetSlot(styleEl) {
...
export function clearStylesheetSlot(styleEl) {
...
export function registerStyleSheet(styleEl, cssText) {
    ...
    if (rulesUsePseudo(rules, 'active'))
        stylesheetsWithActive.add(styleEl);
    else
        stylesheetsWithActive.delete(styleEl);
    if (rulesUsePseudo(rules, 'focus'))
        stylesheetsWithFocus.add(styleEl);
    else
        stylesheetsWithFocus.delete(styleEl);
    if (rulesUsePseudo(rules, 'checked'))
        stylesheetsWithChecked.add(styleEl);
    else
        stylesheetsWithChecked.delete(styleEl);
    if (rulesUsePseudo(rules, 'hover'))
        stylesheetsWithHover.add(styleEl);
    else
        stylesheetsWithHover.delete(styleEl);
    if (rulesUseSibling(rules))
        stylesheetsWithSibling.add(styleEl);
    else
 ...
 * the tree). */
export function unregisterStyleSheet(styleEl) {
...
export function someStylesheetUsesActive() { return stylesheetsWithActive.size > 0; }
export function someStylesheetUsesFocus() { return stylesheetsWithFocus.size > 0; }
export function someStylesheetUsesChecked() { return stylesheetsWithChecked.size > 0; }
export function someStylesheetUsesHover() { return stylesheetsWithHover.size > 0; }
 ...
 * full rebuild. */
export function someStylesheetUsesSibling() { return stylesheetsWithSibling.size > 0; }
...
export function invalidateForSiblingCascade(_el) {
    if (stylesheetsWithSibling.size === 0)
        return;
    clearComputedCache();
...
let layoutCacheResetFn = null;
export function setLayoutCacheResetFn(fn) {
    layoutCacheResetFn = fn;
}
function resetLayoutCachesForSiblingCascade() {
    if (layoutCacheResetFn)
        layoutCacheResetFn();
}
...
export function invalidateLiveStyle(el) {
    if (!el) {
        clearComputedCache();
        return;
    }
...
const touchedSinceClear = new Set();
...
const activeRefcount = new WeakMap();
...
export function setPseudoActive(el, on) {
    if (!el)
        return;
    const prev = activeRefcount.get(el) ?? 0;
    const next = on ? prev + 1 : Math.max(0, prev - 1);
    if (next === 0)
        activeRefcount.delete(el);
    else
        activeRefcount.set(el, next);
...
export function setPseudoFocus(el, on) {
    if (!el)
        return;
    if (on)
        focusElements.add(el);
    else
        focusElements.delete(el);
...
export function setPseudoHover(el, on) {
    if (!el)
        return;
    if (on)
        hoverElements.add(el);
    else
        hoverElements.delete(el);
    if (stylesheetsWithHover.size > 0 && someHoverRuleAffectsElement(el)) {
        invalidateLiveStyle(el);
    }
}
export function isPseudoActive(el) { return activeElements.has(el); }
export function isPseudoFocus(el) { return focusElements.has(el); }
export function isPseudoHover(el) { return hoverElements.has(el); }
// =========================================================================
 ...
 * accumulator. */
function applyUaDefaults(computed, tag, el, parentComputed) {
    ...
    switch (tag) {
        ...
        case 'DIALOG': {
            ...
            computed.paddingTop = 16;
            computed.paddingRight = 16;
            computed.paddingBottom = 16;
        ...
        case 'BLOCKQUOTE':
            computed.marginTop = 8;
            computed.marginBottom = 8;
            computed.marginLeft = 24;
            computed.marginRight = 24;
            return;
        case 'FIGURE':
            computed.marginTop = 8;
            computed.marginBottom = 8;
 ...
 */
function applyPresentationalHints(computed, el) {
    ...
    if (tag === 'INPUT') {
        ...
        if (computed.background === undefined) {
            ...
            else if (type !== 'range' && type !== 'color' && type !== 'hidden'
...
function setLenAttr(computed, key, raw) {
    if (!raw)
        return;
 ...
 */
export function getComputedLiveStyle(el) {
    ...
    for (const { rule, pseudo } of matched) {
        ...
        for (const decl of rule.decls) {
            if (decl.prop.startsWith('--'))
                continue; // already in customProps
    ...
    if (inline.color !== undefined)
        computed.color = inline.color;
    if (inline.background !== undefined)
        computed.background = inline.background;
    if (inline.fontFamily !== undefined)
        computed.fontFamily = inline.fontFamily;
    if (inline.fontSize !== undefined)
        computed.fontSize = inline.fontSize;
    if (inline.fontWeight !== undefined)
        computed.fontWeight = inline.fontWeight;
    if (inline.fontStyle !== undefined)
        computed.fontStyle = inline.fontStyle;
    if (inline.textAlign !== undefined)
        computed.textAlign = inline.textAlign;
    if (inline.lineHeight !== undefined)
        computed.lineHeight = inline.lineHeight;
    if (inline.textDecoration !== undefined)
        computed.textDecoration = inline.textDecoration;
    if (inline.verticalAlign !== undefined)
        computed.verticalAlign = inline.verticalAlign;
    if (inline.listStyleType !== undefined)
        computed.listStyleType = inline.listStyleType;
    if (inline.cursor !== undefined)
        computed.cursor = inline.cursor;
    if (inline.opacity !== undefined)
        computed.opacity = inline.opacity;
    if (inline.display !== undefined)
        computed.display = inline.display;
    if (inline.width !== undefined)
        computed.width = inline.width;
    if (inline.height !== undefined)
        computed.height = inline.height;
    if (inline.paddingTop !== undefined)
        computed.paddingTop = inline.paddingTop;
    if (inline.paddingRight !== undefined)
        computed.paddingRight = inline.paddingRight;
    if (inline.paddingBottom !== undefined)
        computed.paddingBottom = inline.paddingBottom;
    if (inline.paddingLeft !== undefined)
        computed.paddingLeft = inline.paddingLeft;
    if (inline.marginTop !== undefined)
        computed.marginTop = inline.marginTop;
    if (inline.marginRight !== undefined)
        computed.marginRight = inline.marginRight;
    if (inline.marginBottom !== undefined)
        computed.marginBottom = inline.marginBottom;
    if (inline.marginLeft !== undefined)
        computed.marginLeft = inline.marginLeft;
    if (inline.gap !== undefined)
        computed.gap = inline.gap;
    if (inline.flexDirection !== undefined)
        computed.flexDirection = inline.flexDirection;
    if (inline.flexGrow !== undefined)
        computed.flexGrow = inline.flexGrow;
    if (inline.flexShrink !== undefined)
        computed.flexShrink = inline.flexShrink;
    if (inline.flexBasis !== undefined)
        computed.flexBasis = inline.flexBasis;
    if (inline.alignItems !== undefined)
        computed.alignItems = inline.alignItems;
    if (inline.justifyContent !== undefined)
        computed.justifyContent = inline.justifyContent;
    if (inline.boxSizing !== undefined)
        computed.boxSizing = inline.boxSizing;
    if (inline.minWidth !== undefined)
        computed.minWidth = inline.minWidth;
    if (inline.maxWidth !== undefined)
        computed.maxWidth = inline.maxWidth;
    if (inline.minHeight !== undefined)
        computed.minHeight = inline.minHeight;
    if (inline.maxHeight !== undefined)
        computed.maxHeight = inline.maxHeight;
    if (inline.overflowX !== undefined)
        computed.overflowX = inline.overflowX;
    if (inline.overflowY !== undefined)
        computed.overflowY = inline.overflowY;
    if (inline.borderTopWidth !== undefined)
        computed.borderTopWidth = inline.borderTopWidth;
    if (inline.borderRightWidth !== undefined)
        computed.borderRightWidth = inline.borderRightWidth;
    if (inline.borderBottomWidth !== undefined)
        computed.borderBottomWidth = inline.borderBottomWidth;
    if (inline.borderLeftWidth !== undefined)
        computed.borderLeftWidth = inline.borderLeftWidth;
    if (inline.borderTopColor !== undefined)
        computed.borderTopColor = inline.borderTopColor;
    if (inline.borderRightColor !== undefined)
        computed.borderRightColor = inline.borderRightColor;
    if (inline.borderBottomColor !== undefined)
        computed.borderBottomColor = inline.borderBottomColor;
    if (inline.borderLeftColor !== undefined)
        computed.borderLeftColor = inline.borderLeftColor;
    if (inline.borderRadius !== undefined)
        computed.borderRadius = inline.borderRadius;
    if (inline.position !== undefined)
        computed.position = inline.position;
    if (inline.top !== undefined)
        computed.top = inline.top;
    if (inline.left !== undefined)
        computed.left = inline.left;
    if (inline.right !== undefined)
        computed.right = inline.right;
    if (inline.bottom !== undefined)
        computed.bottom = inline.bottom;
    if (inline.zIndex !== undefined)
        computed.zIndex = inline.zIndex;
    // 4. Inherit unset inheritable props from parent's computed.
    if (parentComputed) {
        if (computed.color === undefined && parentComputed.color !== undefined)
            computed.color = parentComputed.color;
        if (computed.fontFamily === undefined && parentComputed.fontFamily !== undefined)
            computed.fontFamily = parentComputed.fontFamily;
        if (computed.fontSize === undefined && parentComputed.fontSize !== undefined)
            computed.fontSize = parentComputed.fontSize;
        if (computed.fontWeight === undefined && parentComputed.fontWeight !== undefined)
            computed.fontWeight = parentComputed.fontWeight;
        if (computed.fontStyle === undefined && parentComputed.fontStyle !== undefined)
            computed.fontStyle = parentComputed.fontStyle;
        if (computed.textAlign === undefined && parentComputed.textAlign !== undefined)
            computed.textAlign = parentComputed.textAlign;
        if (computed.lineHeight === undefined && parentComputed.lineHeight !== undefined)
            computed.lineHeight = parentComputed.lineHeight;
        ...
        if (computed.whiteSpace === undefined && parentComputed.whiteSpace !== undefined)
            computed.whiteSpace = parentComputed.whiteSpace;
        ...
        if (computed.textDecoration === undefined && parentComputed.textDecoration !== undefined)
            computed.textDecoration = parentComputed.textDecoration;
        ...
        if (computed.listStyleType === undefined && parentComputed.listStyleType !== undefined)
            computed.listStyleType = parentComputed.listStyleType;
        if (computed.cursor === undefined && parentComputed.cursor !== undefined)
            computed.cursor = parentComputed.cursor;
        ...
        if (computed.fill === undefined && parentComputed.fill !== undefined)
            computed.fill = parentComputed.fill;
        if (computed.stroke === undefined && parentComputed.stroke !== undefined)
            computed.stroke = parentComputed.stroke;
        if (computed.strokeWidth === undefined && parentComputed.strokeWidth !== undefined)
            computed.strokeWidth = parentComputed.strokeWidth;
    }
    ...
    if (Object.keys(customProps).length > 0)
        computed.customProps = customProps;
    if (beforeContent !== undefined)
        computed.before = beforeContent;
    if (afterContent !== undefined)
        computed.after = afterContent;
    if (beforeStyle)
        computed.beforeStyle = beforeStyle;
    if (afterStyle)
        computed.afterStyle = afterStyle;
    computedCache.set(el, computed);
    touchedSinceClear.add(el);
    return computed;
}
function applyDeclToComputed(computed, customProps, decl, el, parentComputed) {
    ...
    switch (prop) {
        ...
        case 'fill':
            ...
            return;
        case 'stroke':
            computed.stroke = value;
            return;
        case 'stroke-width':
            computed.strokeWidth = value;
            return;
        case 'background':
        case 'background-color': {
            ...
            if (layers)
                ...
                computed.backgroundLayers = undefined;
            return;
        }
        ...
        case 'font-family':
            computed.fontFamily = stripQuoted(value);
            return;
        case 'font-size': {
            ...
            if (kw !== undefined) {
                computed.fontSize = kw;
                return;
            }
            const parentSize = parentComputed?.fontSize ?? 16;
            if (v === 'larger') {
                computed.fontSize = Math.round(parentSize * 1.2);
                return;
            }
            if (v === 'smaller') {
                computed.fontSize = Math.round(parentSize / 1.2);
        ...
        case 'font-style': {
            ...
            if (v === 'italic' || v === 'oblique' || v === 'normal')
                computed.fontStyle = v;
            return;
        }
        case 'text-align': {
            const v = value.trim().toLowerCase();
            if (v === 'left' || v === 'center' || v === 'right' || v === 'start' || v === 'end')
                computed.textAlign = v;
            return;
        }
        ...
        case 'opacity': {
            ...
            if (Number.isFinite(n))
                computed.opacity = Math.max(0, Math.min(1, n));
            return;
        }
        case 'display': {
            const v = value.trim().toLowerCase();
            if (v === 'block' || v === 'inline' || v === 'inline-block'
                || v === 'flex' || v === 'inline-flex' || v === 'grid' || v === 'none')
                computed.display = v;
            return;
        }
        case 'float': {
        ...
        case 'align-items': {
            ...
            if (v === 'stretch' || v === 'flex-start' || v === 'flex-end' || v === 'center') {
                ...
                computed.alignItems = 'flex-start';
            else if (v === 'end')
                computed.alignItems = 'flex-end';
            return;
        }
        case 'justify-items': {
        ...
        case 'place-items': {
            ...
            else if (a === 'start' || a === 'flex-start')
                computed.alignItems = 'flex-start';
            else if (a === 'end' || a === 'flex-end')
                computed.alignItems = 'flex-end';
            else if (a === 'stretch')
        ...
        case 'transform': {
            ...
            if (tf)
                computed.transform = tf;
            return;
        }
        case 'animation': {
            const an = parseAnimationShorthand(value);
            if (an)
                computed.animation = an;
            return;
        }
        case 'animation-name': {
            const trimmed = value.trim();
        ...
        case 'animation-iteration-count': {
            const v = value.trim().toLowerCase();
            const iter = v === 'infinite' ? 'infinite' : Math.max(0, parseFloat(v) || 1);
            computed.animation = {
                name: computed.animation?.name ?? '',
                durationMs: computed.animation?.durationMs ?? 0,
                iterationCount: iter,
        ...
        case 'border-width': {
            ...
            if (n === undefined)
                return;
            computed.borderTopWidth = n;
            computed.borderRightWidth = n;
            computed.borderBottomWidth = n;
            computed.borderLeftWidth = n;
            return;
        }
        case 'border-color': {
            computed.borderTopColor = value;
        ...
        case 'box-shadow': {
            ...
            if (shadows)
                computed.boxShadow = shadows;
            else if (value.trim().toLowerCase() === 'none')
                computed.boxShadow = undefined;
            return;
        }
        case 'border-radius': {
            ...
            if (pct) {
                ...
                if (Number.isFinite(n))
                    computed.borderRadius = { percent: n / 100 };
                return;
            }
            ...
            if (px) {
                ...
                if (Number.isFinite(n))
                    computed.borderRadius = { px: Math.round(n) };
            }
            return;
        }
    }
...
function applyDeclToPseudoStyle(slot, decl, parentComputed, customProps, el) {
    const value = resolveVarRefs(decl.value, el, parentComputed, customProps);
    const emBase = parentComputed?.fontSize ?? 16;
    switch (decl.prop) {
        case 'position': {
            const v = value.trim().toLowerCase();
            if (v === 'static' || v === 'relative' || v === 'absolute')
                slot.position = v;
            return;
        }
        case 'top': {
            assignPseudoEdge(slot, 'top', value, emBase);
            return;
        }
        case 'right': {
            assignPseudoEdge(slot, 'right', value, emBase);
            return;
        }
        case 'bottom': {
            assignPseudoEdge(slot, 'bottom', value, emBase);
            return;
        }
        case 'left': {
            assignPseudoEdge(slot, 'left', value, emBase);
            return;
        }
        ...
        case 'background-color': {
            ...
            const layers = parseBackgroundLayers(value);
            if (layers)
                slot.backgroundLayers = layers;
            return;
        }
        case 'background-size': {
            slot.backgroundSize = parseBackgroundSize(value);
            return;
        }
        case 'opacity': {
            const n = parseFloat(value);
            if (Number.isFinite(n))
                slot.opacity = Math.max(0, Math.min(1, n));
            return;
        }
        case 'width': {
            const len = parseLength(value);
        ...
        case 'font-family':
            slot.fontFamily = stripQuoted(value);
            return;
        case 'font-weight': {
            const v = value.trim().toLowerCase();
            if (v === 'bold')
                slot.fontWeight = 'bold';
            else if (v === 'normal')
                slot.fontWeight = 'normal';
            else {
                const n = parseInt(v, 10);
                if (Number.isFinite(n))
                    slot.fontWeight = n;
...
function applyBorderShorthandToComputed(computed, prop, value) {
    ...
    const setSide = (side) => {
        computed['border' + side + 'Width'] = widthVal;
        if (color !== undefined)
...
function parseOverflowVal(value) {
    const v = value.trim().toLowerCase();
    if (v === 'visible' || v === 'hidden' || v === 'scroll' || v === 'auto')
        return v;
    return undefined;
}
function assignNum(computed, key, value) {
    const n = parsePxOrNum(value);
    if (n !== undefined)
        computed[key] = n;
}
function assignLen(computed, key, value) {
    const v = parseLength(value);
    if (v !== undefined)
        computed[key] = v;
}
function assignBoxComputed(computed, prefix, value) {
    const tokens = value.trim().split(/\s+/).filter(Boolean);
    if (tokens.length === 0)
        return;
    const nums = [];
    ...
    computed[cap.toLowerCase() === 'padding' ? 'paddingTop' : 'marginTop'] = t;
    computed[cap.toLowerCase() === 'padding' ? 'paddingRight' : 'marginRight'] = r;
    computed[cap.toLowerCase() === 'padding' ? 'paddingBottom' : 'marginBottom'] = b;
    computed[cap.toLowerCase() === 'padding' ? 'paddingLeft' : 'marginLeft'] = l;
}
...
function expandInsetTokens(value) {
    const tokens = value.trim().split(/\s+/).filter(Boolean);
    if (tokens.length === 0)
        return undefined;
    const nums = [];
 ...
 * leave both. */
function assignPosEdge(computed, edge, value) {
    const t = value.trim();
    const pct = /^(-?\d+(?:\.\d+)?)%$/.exec(t);
    const pctKey = (edge + 'Pct');
    if (pct) {
...
function parseTransformTranslate(value) {
    let tx;
    let ty;
    let rotateRad;
    let scaleX;
...
function parseAnimationShorthand(value) {
    ...
    for (const p of parts) {
        ...
        if (/^[\d.]+$/.test(lower)) {
            iterationCount = Math.max(0, parseFloat(lower));
            continue;
        }
...
function parseKeyframeSelectorList(prelude) {
    const out = [];
...
function fetchFontBytesSync(url) {
 ...
 */
async function fetchFontBytesAsync(url) {
    const resp = await fetch(url);
    if (!resp.ok)
        throw new Error('http ' + resp.status + ' for ' + url);
    return await resp.arrayBuffer();
}
function addFontFaceToSet(family, bytes, source) {
    try {
        ...
        fonts.add(face);
        console.debug('[font-face] registered family=' + family + ' bytes=' + bytes.byteLength + ' source=' + source);
    }
    catch (err) {
        console.debug('[font-face] FontFace ctor threw for family=' + family + ': ' + (err?.message || err));
    }
}
function extractFirstUrl(src) {
...
function registerFontFace(blockNode) {
    let family;
    let src;
    const block = blockNode;
    if (!block.children || typeof block.children.forEach !== 'function')
        return;
    ...
    if (registeredFontFamilies.has(key))
        return; // already registered (or fetch already in flight)
    ...
    fetchFontBytesAsync(url).then((b) => addFontFaceToSet(familyResolved, b, 'async:' + url), (err) => {
        console.debug('[font-face] async fetch failed family=' + familyResolved + ' url=' + url + ': ' + (err?.message || err));
        registeredFontFamilies.delete(key); // allow caller to retry
    });
}
function registerKeyframes(name, blockNode) {
    const stops = [];
    const block = blockNode;
    if (!block.children || typeof block.children.forEach !== 'function')
        return;
    block.children.forEach((kf) => {
        ...
        if (offsets.length === 0 || !kfRule.block)
            return;
        const decls = Object.create(null);
        const inner = kfRule.block;
        ...
        if (decls['transform']) {
            ...
            if (tf) {
                if (tf.rotateRad !== undefined)
                    stopBase.rotateRad = tf.rotateRad;
                if (tf.scaleX !== undefined)
                    stopBase.scaleX = tf.scaleX;
                if (tf.scaleY !== undefined)
                    stopBase.scaleY = tf.scaleY;
            }
        }
        if (decls['opacity']) {
        ...
        for (const off of offsets)
            stops.push({ offset: off, ...stopBase });
    });
    stops.sort((a, b) => a.offset - b.offset);
    if (stops.length > 0)
        keyframesRegistry.set(name, stops);
}
/** Normalize a `justify-items` / `place-items` inline-axis token. `flex-*`
 * spellings map to start/end; unrecognized → undefined. */
function parseJustifyItems(value) {
    const v = value.trim().toLowerCase();
    if (v === 'center' || v === 'stretch' || v === 'start' || v === 'end')
        return v;
    if (v === 'flex-start' || v === 'left')
        return 'start';
    if (v === 'flex-end' || v === 'right')
        return 'end';
    return undefined;
}
function assignInset(computed, value) {
    ...
    if (bottom !== undefined)
        computed.bottom = bottom;
    if (left !== undefined)
        computed.left = left;
}
function assignInsetPseudo(slot, value) {
    const edges = expandInsetTokens(value);
    if (!edges)
        return;
    const [top, right, bottom, left] = edges;
...
function parseMaskImage(value) {
    const t = value.trim();
    const lin = /^linear-gradient\((.+)\)$/i.exec(t);
    if (lin)
        return parseLinearGradient(lin[1]);
    const rad = /^radial-gradient\((.+)\)$/i.exec(t);
    if (rad)
        return parseRadialGradient(rad[1]);
    return undefined;
}
function assignFlexShorthand(computed, value) {
    const tokens = value.trim().split(/\s+/).filter(Boolean);
    if (tokens.length === 0)
        return;
    if (tokens.length === 1) {
    ...
    if (g !== undefined)
        computed.flexGrow = g;
    if (s !== undefined)
        computed.flexShrink = s;
    if (tokens.length >= 3) {
        const b = parsePxOrNum(tokens[2]);
        if (b !== undefined)
            computed.flexBasis = b;
    }
}
...
function resolveVarRefs(value, el, parentComputed, ownCustomProps) {
    ...
    for (let i = 0; i < 8 && out.indexOf('var(') >= 0; i++) {
        const next = out.replace(/var\(\s*(--[a-z0-9-]+)\s*(?:,\s*([^)]+))?\s*\)/gi, (_m, name, fallback) => {
            if (ownCustomProps && name in ownCustomProps)
                return ownCustomProps[name];
    ...
    // 21px square.
    if (out.indexOf('calc(') >= 0)
        out = evaluateCalcExpressions(out);
    return out;
}
...
function evaluateCalcExpressions(value) {
    let out = '';
    let i = 0;
...
function lookupVar(_el, name, parentComputed) {
...
function fillGradientStopPositions(stops) {
    if (stops.length === 0)
        return;
    const positioned = (s) => s.pos !== undefined || s.posPx !== undefined;
    if (!positioned(stops[0]))
        stops[0].pos = 0;
    if (!positioned(stops[stops.length - 1]))
    ...
    while (i < stops.length) {
        if (positioned(stops[i])) {
            i++;
            continue;
        }
        let j = i;
        while (j < stops.length && !positioned(stops[j]))
            j++;
...
function parseRadialGradient(inner) {
    const parts = splitTopLevelCommas(inner);
    if (parts.length === 0)
        return undefined;
    let shape = 'ellipse';
    ...
    if (!looksLikeColor && (head.includes('circle') || head.includes('ellipse') || head.includes(' at '))) {
        if (/\bcircle\b/.test(head))
            shape = 'circle';
        else if (/\bellipse\b/.test(head))
            shape = 'ellipse';
...
export function parseBackgroundLayers(value) {
    ...
    for (const part of parts) {
        ...
        if (urlM) {
            ...
            // Repeat keywords.
            if (/\bno-repeat\b/i.test(rest))
                img.repeat = 'no-repeat';
            else if (/\brepeat-x\b/i.test(rest))
                img.repeat = 'repeat-x';
            else if (/\brepeat-y\b/i.test(rest))
                img.repeat = 'repeat-y';
            else if (/\brepeat\b/i.test(rest))
                img.repeat = 'repeat';
            // Position keyword (single — we don't track per-axis yet).
            if (/\bcenter\b/i.test(rest))
                img.position = 'center';
            else if (/\bleft\b/i.test(rest))
                img.position = 'left';
            else if (/\bright\b/i.test(rest))
                img.position = 'right';
            else if (/\btop\b/i.test(rest))
                img.position = 'top';
            else if (/\bbottom\b/i.test(rest))
                img.position = 'bottom';
            ...
            // override.
            if (/\bcover\b/i.test(rest))
                img.sizeMode = 'cover';
            else if (/\bcontain\b/i.test(rest))
                img.sizeMode = 'contain';
            else {
                const sizeMatch = /\/\s*(auto|(\d+(?:\.\d+)?)px)\s+(auto|(\d+(?:\.\d+)?)px)/i.exec(rest);
                if (sizeMatch) {
                    img.sizeMode = 'auto';
                    if (sizeMatch[2])
                        img.sizeW = parseFloat(sizeMatch[2]);
                    if (sizeMatch[4])
                        img.sizeH = parseFloat(sizeMatch[4]);
                }
            }
            layers.push(img);
            sawRichLayer = true;
...
function assignPseudoEdge(slot, edge, value, emBase) {
    const t = value.trim();
    const pct = /^(-?\d+(?:\.\d+)?)%$/.exec(t);
    if (pct) {
        const n = parseFloat(pct[1]);
    ...
    if (n !== undefined) {
        slot[edge] = n;
        slot[edge + 'Pct'] = undefined;
    }
}
function parsePxOrNum(value, ctx) {
    const t = value.trim();
    ...
    if (vh)
        return parseFloat(vh[1]) * 0.01 * getCssViewport().h;
    const vw = /^(-?\d+(?:\.\d+)?)vw$/.exec(t);
    if (vw)
        return parseFloat(vw[1]) * 0.01 * getCssViewport().w;
    ...
    if (rem)
        return parseFloat(rem[1]) * (ctx?.remBase ?? 16);
    const em = /^(-?\d+(?:\.\d+)?)em$/.exec(t);
    if (em)
        return parseFloat(em[1]) * (ctx?.emBase ?? 16);
...
// =========================================================================
function parseStyleSheet(cssText, baseSource) {
    const out = [];
    let ast;
    try {
 ...
 * profile. */
function walkRulesWithMedia(node, mediaActive, visit) {
    ...
    if (node.type === 'Atrule') {
        ...
        if (name === 'media') {
            ...
            if (at.block)
                walkRulesWithMedia(at.block, innerActive, visit);
            return;
        }
        if (name === 'keyframes' || name === '-webkit-keyframes') {
            const animName = at.prelude ? safeGenerate(at.prelude).trim() : '';
            if (animName && at.block)
                registerKeyframes(animName, at.block);
            return;
        }
        if (name === 'font-face') {
            if (at.block)
                registerFontFace(at.block);
            return;
        }
...
let mediaVpW = 1280;
let mediaVpH = 720;
export function setMediaViewport(w, h) {
    mediaVpW = w;
    mediaVpH = h;
}
 ...
 * `registerStyleSheet`. */
let mediaColorScheme = 'light';
export function setMediaColorScheme(scheme) {
 ...
 */
function matchMediaQuery(q) {
    const raw = q.toLowerCase();
    const t = raw.replace(/\s+/g, '');
    if (!t || t === 'all' || t === 'screen')
        return true;
    if (t.indexOf('(hover:hover)') >= 0)
        return false;
    if (t.indexOf('(hover:none)') >= 0)
        return true;
    if (t.indexOf('(pointer:coarse)') >= 0)
        return true;
    if (t.indexOf('(pointer:fine)') >= 0)
        return false;
    ...
    if (t.indexOf('(prefers-color-scheme:light)') >= 0)
        return mediaColorScheme === 'light';
    if (t.indexOf('(prefers-color-scheme:dark)') >= 0)
        return mediaColorScheme === 'dark';
    if (t.indexOf('(prefers-color-scheme:no-preference)') >= 0)
        return false;
...
function collectRule(rule, out, mediaActive, baseSource) {
...
function parseAttrSelector(node) {
    ...
    const matcher = (node.matcher ?? 'has');
    if (matcher !== 'has' && matcher !== '=' && matcher !== '~=' &&
                ...
                if (c.type === 'SelectorList') {
                    ...
                    list.children.forEach((sub) => {
                        ...
                        if (sub.type === 'Selector') {
                            const chain = parseSelectorChain(sub);
...
function computeChainSpecificity(chain) {
    let id = 0;
    let cls = 0;
    let type = 0;
    for (const compound of chain.compounds) {
    ...
    if (chain.pseudoElement)
        type++;
    return [id, cls, type];
}
function matchChain(chain, el, vp) {
    const { compounds, combinators } = chain;
    if (!matchCompound(compounds[compounds.length - 1], el, vp))
        return false;
    let current = el;
    for (let i = compounds.length - 2; i >= 0; i--) {
        ...
        if (combinator === '>') {
            const p = current?.parent ?? null;
            if (!p || !matchCompound(left, p, vp))
                return false;
            current = p;
        }
        else if (combinator === '+') {
            ...
            if (!sib || !matchCompound(left, sib, vp))
                return false;
            current = sib;
        }
        else if (combinator === '~') {
            ...
            let sib = prevElementSibling(current);
            let matched = false;
            while (sib) {
...
function prevElementSibling(el) {
    ...
    for (let i = idx - 1; i >= 0; i--) {
        if (siblings[i].tagName !== '#text')
            return siblings[i];
    }
    return null;
}
function matchCompound(compound, el, vp) {
    if (compound.tag && compound.tag !== el.tagName.toLowerCase())
        return false;
    if (compound.id && el.attrs.id !== compound.id)
        return false;
    ...
    for (const pseudo of compound.pseudos) {
        if (!matchPseudo(pseudo, el, vp))
            return false;
    }
    return true;
}
function matchAttr(attr, el) {
...
function matchPseudo(pseudo, el, vp) {
    switch (pseudo.kind) {
        case 'active':
            if (vp && vp.activeOff === el)
                return false;
            if (vp && vp.activeOn === el)
                return true;
            return activeElements.has(el);
        case 'focus':
            if (vp && vp.focusOff === el)
                return false;
            if (vp && vp.focusOn === el)
                return true;
        ...
        case 'nth': {
            ...
            for (const sib of parent.children) {
                if (sib.tagName === '#text')
                    continue;
                if (pseudo.ofType && sib.tagName !== el.tagName)
                    continue;
                siblings.push(sib);
            }
...
//# sourceMappingURL=live-css.js.map