...
export function populateLiveRoot(parsed) {
 ...
 * later).
 */
export function populateRootFromTree(target, parsed, scope) {
    const byParsed = new Map();
    if (scope) {
...
export function parseFragmentInto(target, html) {
 ...
 * reference). */
const pendingHeadLinks = new WeakMap();
...
function attachHeadStyles(node, liveRoot, scope) {
    const links = [];
    const visit = (n) => {
        if (n.tag === 'body')
            return; // body styles register via the normal walk
        if (n.tag === 'style') {
            let cssText = '';
    ...
    visit(node);
    if (links.length > 0)
        pendingHeadLinks.set(node, links);
}
...
export async function loadHeadLinkStylesheets(parsed, pageUrl) {
...
export function isExternalCssLoading() {
    return __externalCssLoadingCount > 0;
}
export async function loadHeadLinkStylesheetsWithFlag(parsed, pageUrl) {
...
function convertElement(source, byParsed) {
 ...
 * dropped silently. */
function appendConverted(parent, source, byParsed) {
    if (source.type === 'text') {
        if (!source.text)
            return;
...
export function loadIframeContents(iframeEl, src) {
    if (!src)
        return;
    const state = iframeEl;
    if (state._iframeLoadInFlight)
        return;
...
function attachIframeHeadStyles(node, iframeEl, scope) {
    const visit = (n) => {
        if (n.tag === 'body')
            return;
        if (n.tag === 'style') {
...
function appendIframeChild(parent, source, byParsed, scope) {
    if (source.type === 'text') {
        if (!source.text)
            return;
        const textNode = new LiveElement('#text');
    ...
    if (source.tag === 'script')
        return; // never run iframe scripts
    if (SKIP_TAGS.has(source.tag) && source.tag !== 'style')
        return;
    parent.appendChild(convertIframeElement(source, byParsed, scope));
}
function convertIframeElement(source, byParsed, scope) {
    const live = new LiveElement(source.tag);
    byParsed.set(source, live);
    for (const [name, value] of Object.entries(source.attrs)) {
        live.setAttribute(name, value);
    ...
    if (source.tag === 'style') {
        ...
        if (cssText)
            live.textContent = scopeCssToSelector(cssText, scope);
        return live;
    }
    for (const child of source.children) {
...
function rewriteIframeSelector(sel, scope) {
...
export function scopeCssToSelector(cssText, scope) {
...
//# sourceMappingURL=html-to-live.js.map