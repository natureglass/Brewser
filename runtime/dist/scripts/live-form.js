...
import { invalidateForSiblingCascade, someStylesheetUsesSibling } from './live-css.js';
import { bumpLiveTreeVersion, getLiveRoot } from './live-dom.js';
import { getLayoutBox } from './live-layout.js';
...
let keyboardOpener = null;
export function setKeyboardOpener(fn) {
    keyboardOpener = fn;
}
const LIGHT_THEME = {
    textFieldBg: '#ffffff',
...
let liveFormTheme = LIGHT_THEME;
export function setLiveFormColorScheme(_scheme) {
...
export function setInputValue(el, v) {
    valueMap.set(el, v);
...
export function getInputChecked(el) {
    const stored = checkedMap.get(el);
    if (stored !== undefined)
        return stored;
    return el.hasAttribute('checked');
}
export function setInputChecked(el, v) {
    checkedMap.set(el, v);
    bumpLiveTreeVersion();
}
...
export function isFormWidget(el) {
    const t = el.tagName;
...
export function paintFormWidget(ctx, el, cs, box, 
...
skipBg = false) {
    ...
    if (tag === 'INPUT') {
        ...
        switch (type) {
            case 'checkbox':
                paintCheckbox(ctx, el, cs, box);
                return true;
            case 'radio':
                paintRadio(ctx, el, cs, box);
                return true;
            case 'range':
                paintRange(ctx, el, cs, box);
                return true;
            case 'color':
                paintColorSwatch(ctx, el, cs, box);
                return true;
            case 'button':
            case 'submit':
                paintButton(ctx, el, cs, box, skipBg);
                return true;
            case 'file':
                paintFileInput(ctx, el, cs, box, skipBg);
                return true;
            default:
                paintTextField(ctx, el, cs, box, skipBg);
                return true;
        }
    }
    if (tag === 'BUTTON') {
...
function paintCheckbox(ctx, el, cs, box) {
...
function paintRadio(ctx, el, cs, box) {
    const checked = getInputChecked(el);
 ...
 * `handleFormTap`. */
function paintRange(ctx, el, cs, box) {
...
function paintColorSwatch(ctx, el, cs, box) {
...
function formBorderRadius(cs, w, h) {
...
function strokeRoundedRectPath(ctx, x, y, w, h, r) {
    const cr = Math.min(r, w / 2, h / 2);
    ctx.beginPath();
    ctx.moveTo(x + cr, y);
    ctx.lineTo(x + w - cr, y);
...
function paintButton(ctx, el, cs, box, skipBg = false) {
...
function paintFileInput(ctx, el, cs, box, skipBg = false) {
...
function paintTextField(ctx, el, cs, box, skipBg = false) {
    const color = cs.color || liveFormTheme.textFieldText;
...
function drawWrappedText(ctx, text, x, y, maxWidth, lineH) {
    let cursorY = y;
...
function readDescendantText(el) {
    if (el.textContent)
        return el.textContent;
    let s = '';
    const visit = (e) => {
...
function paintSelect(ctx, el, cs, box) {
    ...
    if (showBorder) {
        ...
        if (radius > 0) {
            ...
            strokeRoundedRectPath(ctx, box.x + 0.5, box.y + 0.5, box.w - 1, box.h - 1, Math.max(0, radius - 0.5));
        }
        else {
            ctx.strokeRect(box.x + 0.5, box.y + 0.5, box.w - 1, box.h - 1);
        }
...
export async function handleFormTap(el, tapX, clickAlreadyFired = false) {
    ...
    if (tag === 'LABEL') {
        ...
        let target = null;
        if (forId)
            target = findById(getLiveRoot(), forId);
        if (!target)
            target = findFirstFormWidget(el);
        if (target && target !== el) {
...
function buildKeyboardValidator(el) {
    if (el.tagName !== 'INPUT')
        return null;
    const type = inputType(el);
    if (type !== 'number')
...
function sanitizeKeyboardResult(el, raw) {
    if (el.tagName !== 'INPUT')
        return raw;
    const type = inputType(el);
    if (type === 'number') {
...
export async function openKeyboardAndApply(el) {
    if (!keyboardOpener)
        return false;
    const cur = getInputValue(el);
...
function findFirstFormWidget(root) {
    for (const c of root.children) {
        if (isFormWidget(c))
            return c;
...
function findRadioGroup(root, name) {
    const out = [];
    const visit = (el) => {
        if (el.tagName === 'INPUT'
...
export function findEnclosingForm(start) {
 ...
 */
export function buildFormSubmitUrl(form, submitter) {
...
function appendControlValue(el, submitter, out) {
    const name = el.getAttribute('name');
    if (!name)
        return;
    if (el.hasAttribute('disabled'))
...
//# sourceMappingURL=live-form.js.map