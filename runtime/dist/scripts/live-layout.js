...
import { getComputedLiveStyle, setLayoutCacheResetFn } from './live-css.js';
import { quoteFontFamily, resolveLength } from './inline-css.js';
 ...
 * context. */
let measureCtx = null;
export function setLayoutMeasureCtx(ctx) {
    measureCtx = ctx;
}
...
const inlineCacheTouched = new Set();
...
export function getLayoutBox(el) {
    return cache.get(el);
}
...
export function getInlineLayout(el) {
 ...
 * defaults.
 */
export function layoutFixedRoot(root, originX, originY, availableWidth, availableHeight) {
 ...
 * deterministically. */
export function collectAbsolutes(root) {
    const out = [];
    const visit = (el) => {
...
export function findAbsoluteContainingBlock(el, fallbackRoot) {
    let cur = el.parent;
    while (cur) {
...
export function layoutAbsoluteRoot(el, cbContentX, cbContentY, cbContentW, cbContentH) {
    const cs = getComputedLiveStyle(el);
    ...
    const edge = (px, pct, basis) => px !== undefined ? px : pct !== undefined ? (pct / 100) * basis : undefined;
    ...
    if (csLeft !== undefined)
        x = cbContentX + csLeft;
    else if (csRight !== undefined)
        x = cbContentX + cbContentW - box.w - csRight;
    let y = cbContentY;
    if (csTop !== undefined)
        y = cbContentY + csTop;
    else if (csBottom !== undefined)
        y = cbContentY + cbContentH - box.h - csBottom;
    ...
    if (tf) {
        if (tf.tx !== undefined)
            x += resolveLength(tf.tx, box.w) ?? 0;
        if (tf.ty !== undefined)
            y += resolveLength(tf.ty, box.h) ?? 0;
    }
...
function isLonelyStateInput(el) {
    if (el.tagName !== 'INPUT')
        return false;
 ...
 */
function layoutChildren(parent, originX, originY, contentW, contentH) {
    ...
    const kids = parent.children.filter((c) => {
        ...
        if (ccs.display === 'none')
            return false;
        if (isClosedDetails && c.tagName !== 'SUMMARY')
            return false;
    ...
    for (const c of kids) {
        ...
        if (d !== 'inline' && d !== 'inline-block' && !isReplacedAtom) {
            allInline = false;
            break;
        }
    }
 ...
 */
const fieldsetLegendCutouts = new WeakMap();
/**
 ...
 */
export function getFieldsetLegendCutout(el) {
    return fieldsetLegendCutouts.get(el);
}
function layoutFieldsetWithLegend(parent, kids, cs, originX, originY, contentW, contentH) {
    const legend = kids[0];
    ...
    const legendW = Math.min(clampSize(explicitW ?? naturalW, lcs.minWidth, lcs.maxWidth, contentW), contentW);
 ...
 */
function layoutInline(parent, kids, parentCs, originX, originY, contentW, contentH) {
    ...
    if (!ctx)
        return 0;
    const work = [];
    function tokenizeText(el, raw, font, fontSize) {
        if (!raw)
            return;
        ctx.save();
        ctx.font = font;
        try {
            ...
            while (i < raw.length) {
                ...
                if (c <= 32) {
                    while (i < raw.length && raw.charCodeAt(i) <= 32)
                        i++;
                    work.push({
                ...
                while (i < raw.length) {
                    ...
                    if (cc <= 32)
                        break;
                    if (scanForEmoji && cc >= 0x80 && matchEmojiClusterAt(raw, i))
                        break;
                    i++;
                }
                if (i > start) {
    ...
    function walkInline(el, cs) {
        ...
        if (el.tagName === 'IMG') {
            ...
            else if (styleW !== undefined) {
                w = styleW;
                h = nh > 0 ? styleW * (nh / Math.max(1, nw)) : styleW;
            }
            else if (styleH !== undefined) {
                h = styleH;
                w = nw > 0 ? styleH * (nw / Math.max(1, nh)) : styleH;
            }
            else if (nw > 0 && nh > 0) {
                w = nw;
                h = nh;
        ...
        if (el.tagName === 'CANVAS') {
            ...
            if (styleW !== undefined && styleH !== undefined) {
                ...
                w = styleW;
                h = ds.w > 0 ? styleW * (ds.h / Math.max(1, ds.w)) : styleW;
            }
            else if (styleH !== undefined) {
                h = styleH;
                w = ds.h > 0 ? styleH * (ds.w / Math.max(1, ds.h)) : styleH;
            }
            else {
                w = ds.w > 0 ? ds.w : 300;
                h = ds.h > 0 ? ds.h : 150;
        ...
        if (cs.before)
            tokenizeText(el, cs.before, font, fontSize);
        if (el.textContent)
            tokenizeText(el, el.textContent, font, fontSize);
        ...
        if (cs.after)
            tokenizeText(el, cs.after, font, fontSize);
        if (pR > 0) {
            work.push({
    ...
    for (const a of work) {
        ...
        if (a.isWhitespace && current.items.length === 0)
            continue;
    ...
    let align = parentCs.textAlign ?? 'start';
    if (parentCs.justifyItems === 'center')
        align = 'center';
    else if (parentCs.justifyItems === 'end')
        align = 'right';
    else if (parentCs.justifyItems === 'start')
        align = 'left';
    const placed = [];
    let y = originY;
    for (const line of lines) {
        ...
        let lineX = originX;
        if (align === 'center')
            lineX = originX + (contentW - line.w) / 2;
        else if (align === 'right' || align === 'end')
            lineX = originX + contentW - line.w;
        let cursor = lineX;
    ...
    const layout = { atoms: placed, height: total };
...
function layoutBlock(kids, originX, originY, contentW, contentH, parentCs) {
    let y = originY;
    ...
    for (const child of kids) {
        ...
        // stacked full-width.
        const isInlinePack = !isFloat && (ccs.display === 'inline-block' || ccs.display === 'inline');
        if (isFloat || isInlinePack) {
            ...
            if (childExplicitWF === undefined && isInlinePack) {
                ...
                if (intrinsic > 0)
                    childExplicitWF = intrinsic;
            }
            if (childExplicitWF !== undefined && childExplicitWF > 0) {
                const childWF = clampSize(childExplicitWF, ccs.minWidth, ccs.maxWidth, contentW);
                ...
                if (consumedH > floatRowH)
                    floatRowH = consumedH;
                if (isFloat && float === 'right')
                    floatRowRightX -= childWF + mLeft + mRight;
                else
                    floatRowLeftX += childWF + mLeft + mRight;
                prevMarginBottom = 0;
                continue;
            }
        }
        ...
        let childW = clampSize((childExplicitW ?? (contentW - mLeft - mRight)), ccs.minWidth, ccs.maxWidth, contentW);
        ...
        if (isReplacedInline && childExplicitW === undefined) {
            if (tag === 'CANVAS') {
                ...
                if (Number.isFinite(attrW) && attrW > 0)
                    childW = attrW;
                else if (naturalW > 0)
                    childW = naturalW;
            }
...
function layoutFlex(parent, kids, parentCs, originX, originY, contentW, contentH) {
    ...
    const items = kids.map((el) => {
        ...
        let basis;
        if (m.basis !== undefined)
            basis = m.basis;
        else if (m.explicit !== undefined)
            basis = m.explicit;
        else {
    ...
    for (const it of orderedItems) {
        ...
        const crossPos = computeCrossOrigin(isRow ? originY : originX, crossAvail, it.crossSize, it.marginCrossStart, it.marginCrossEnd, align);
        let cx, cy, cw, ch;
        if (isRow) {
            cx = mainPos;
            cy = crossPos;
...
function computeCrossOrigin(crossOrigin, crossAvail, crossSize, mStart, mEnd, align) {
...
function layoutGrid(parent, kids, parentCs, originX, originY, contentW, contentH) {
    const gap = parentCs.gap ?? 0;
...
function layoutGridExplicit(parent, kids, parentCs, originX, originY, contentW, contentH, gap) {
    ...
    for (const a of autoKids) {
        while (scan < cellCount && occupied[Math.floor(scan / colCount)][scan % colCount])
            scan++;
...
function parseGridLine(value, _trackCount) {
    if (!value)
        return null;
    ...
    if (spanOnly)
        return { start: 0, span: Math.max(1, parseInt(spanOnly[1], 10)) };
    ...
    if (endTok) {
        ...
        if (spanEnd)
            span = Math.max(1, parseInt(spanEnd[1], 10));
        else {
...
function resolveGridTracks(value, contentW, gap, itemCount) {
    if (!value)
        return [contentW];
    const v = value.trim();
    ...
    if (autoFit) {
        ...
        if (cols < 1)
            cols = 1;
        if (isAutoFit && itemCount > 0 && itemCount < cols)
            cols = itemCount;
...
function parseGridTrackLen(s, basis) {
    const t = s.trim();
    const pct = /^(-?\d+(?:\.\d+)?)%$/.exec(t);
    if (pct)
        return parseFloat(pct[1]) * 0.01 * basis;
...
function parseGridTrackToken(tok, basis) {
    const t = tok.trim();
...
function distributeTrackList(tokens, contentExtent, gap) {
    const totalGap = Math.max(0, tokens.length - 1) * gap;
    const avail = Math.max(0, contentExtent - totalGap);
    const descs = tokens.map((t) => parseGridTrackToken(t, avail) ?? { kind: 'fr', v: 1, min: 0 });
    let frTotal = 0;
    let fixedTotal = 0;
    ...
    return descs.map((d) => d.kind === 'fixed' ? d.v : Math.max(d.min, frTotal > 0 ? frSpace * (d.v / frTotal) : 0));
}
...
function layoutTable(table, kids, tableCs, originX, originY, contentW) {
    const rows = [];
    let caption = null;
    const walkSection = (root) => {
        for (const child of root.children) {
            ...
            if (tag === 'TR') {
                ...
                for (const tc of child.children) {
                    if (tc.tagName === 'TD' || tc.tagName === 'TH')
                        cells.push(tc);
                }
                rows.push(cells);
            }
    ...
    const colMin = new Array(colCount).fill(0);
    const colMax = new Array(colCount).fill(0);
    ...
    for (const row of rows) {
        ...
        for (let c = 0; c < colCount; c++) {
            ...
            if (c < row.length) {
                ...
                const innerH = layoutCellContent(cell, cellCs, contentX, contentYTop, cellContentW);
...
function layoutCellContent(cell, cellCs, originX, originY, contentW) {
...
function measureCellWidthsLive(cell) {
    const ctx = measureCtx;
    if (!ctx)
        return { min: 0, max: 0 };
    const cellCs = getComputedLiveStyle(cell);
...
function distributeColumnWidths(colMin, colMax, available) {
...
function layoutLeaf(el, cs, x, y, w, hHint) {
    try {
        const pad = padding(cs);
        ...
        if (tag === 'IMG') {
            ...
            if (Number.isFinite(attrH) && attrH > 0)
                intrinsicVoidH = attrH;
            else if (naturalH > 0)
                intrinsicVoidH = naturalH;
            else
                intrinsicVoidH = 24;
        ...
        if (tag === 'IFRAME') {
            ...
            if (explicitW === undefined) {
                if (Number.isFinite(wAttr) && wAttr > 0)
                    w = wAttr;
                else
                    w = 300;
            }
        ...
        if (tag === 'SVG') {
            ...
            if (explicitH !== undefined)
                intrinsicVoidH = explicitH;
            else if (Number.isFinite(hAttr) && hAttr > 0)
                intrinsicVoidH = hAttr;
            else {
        ...
        if (tag === 'SELECT') {
            ...
            if (!(sizeAttr > 1)) {
                const h = clampSize(explicitH ?? hHint ?? (pad.top + intrinsicVoidH + pad.bottom), cs.minHeight, cs.maxHeight, heightBasis);
                const contentH = Math.max(0, h - pad.top - pad.bottom);
                storeBox(el, {
                    x, y, w, h,
        ...
        try {
            ...
            console.debug('[live-layout] layoutLeaf threw, using fallback box for', '<' + ((el && el.tagName) || '?').toLowerCase() + (id ? ' id=' + id : '') + (cls ? ' class=' + cls : '') + '>', err);
        }
        catch (_) { /* ignore */ }
        const fb = hHint ?? 0;
        try {
...
function clampSize(v, min, max, basis) {
    const minPx = resolveLength(min, basis);
    const maxPx = resolveLength(max, basis);
    if (maxPx !== undefined && v > maxPx)
        v = maxPx;
    if (minPx !== undefined && v < minPx)
        v = minPx;
    return v;
}
function clampMain(cs, isRow, v, contentW, contentH) {
    if (isRow)
        return clampSize(v, cs.minWidth, cs.maxWidth, contentW);
    return clampSize(v, cs.minHeight, cs.maxHeight, contentH);
}
function clampCross(cs, isRow, v, contentW, contentH) {
    if (isRow)
        return clampSize(v, cs.minHeight, cs.maxHeight, contentH);
    return clampSize(v, cs.minWidth, cs.maxWidth, contentW);
}
function mainAxis(cs, isRow, contentW, contentH) {
    if (isRow) {
        return {
            basis: cs.flexBasis,
...
function crossAxis(cs, isRow, contentW, contentH) {
    if (isRow) {
        return {
            explicit: resolveLength(cs.height, contentH),
            marginStart: cs.marginTop ?? 0,
...
function crossExplicit(cs, isRow) {
...
function intrinsicMain(el, cs, isRow) {
    if (isRow) {
    ...
    if (typeof cs.height === 'number')
        return cs.height;
    if (el.textContent)
        return (cs.fontSize ?? 14) * 1.2 + (cs.paddingTop ?? 0) + (cs.paddingBottom ?? 0);
    return 0;
}
...
function svgIntrinsicContentSize(el, cs) {
    ...
    if (w === undefined && h !== undefined)
        w = vbAspect ? Math.round(h * vbAspect) : h;
    if (h === undefined && w !== undefined)
        h = vbAspect ? Math.round(w / vbAspect) : w;
    return { w: w ?? 16, h: h ?? 16 };
}
function intrinsicContentWidth(el, cs) {
    ...
    if (explicit === undefined && el.tagName === 'IMG') {
        ...
        let iw;
        if (explicitH !== undefined && nW > 0 && nH > 0)
            iw = Math.round((nW / nH) * explicitH);
        else if (nW > 0)
            iw = nW;
        else if (explicitH !== undefined)
            iw = explicitH;
...
function outerIntrinsicWidth(el, cs) {
...
function intrinsicContentHeight(el, cs) {
...
function outerIntrinsicHeight(el, cs) {
 ...
 * canvas children. */
function canvasIntrinsicWidth(root) {
    let w = 0;
    const walk = (node) => {
...
function intrinsicCross(el, cs, isRow) {
    if (el.tagName === 'SVG') {
    ...
    if (isRow) {
        ...
        if (hasElementKids)
            return intrinsicContentHeight(el, cs);
        if (el.textContent)
            return lineH + padV;
...
//# sourceMappingURL=live-layout.js.map