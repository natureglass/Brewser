...
// `WifiWebAuthApplet`.
const SWITCH_USER_AGENT = 'Mozilla/5.0 (Nintendo Switch; WifiWebAuthApplet) AppleWebKit/606.4 (KHTML, like Gecko) NF/6.0.2.20.5 NintendoBrowser/5.1.0.21002';
export class SwitchUaFetchLoader {
    nativeFetch;
    permissionPolicy;
    colorScheme;
    cookieJar;
...
function _cookieDiag(msg) {
    if (!COOKIE_DIAG || _cookieDiagCount >= COOKIE_DIAG_CAP)
        return;
    _cookieDiagCount++;
    try {
...
function describeCookieHeader(init) {
...
function countSetCookieHeaders(headers) {
    const h = headers;
    if (typeof h.getSetCookie === 'function')
        return h.getSetCookie().length;
    let n = 0;
    h.forEach((_value, name) => { if (name.toLowerCase() === 'set-cookie')
        n++; });
    return n;
}
function sampleSetCookieHeader(headers) {
    const h = headers;
    let first;
    if (typeof h.getSetCookie === 'function') {
        ...
        h.forEach((value, name) => {
            if (first === undefined && name.toLowerCase() === 'set-cookie')
                first = value;
        });
    }
    if (!first)
 ...
 * a quoted token. */
function withSwitchHeaders(init, colorScheme) {
...
//# sourceMappingURL=switch-ua-fetch-loader.js.map