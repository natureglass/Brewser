...
// document.
let activeShimDispatcher = null;
let activeShim = null;
function dispatchPointerLockChange() {
    if (!activeShimDispatcher || !activeShim)
        return;
    try {
        activeShimDispatcher({ type: 'pointerlockchange', target: activeShim });
    }
    catch { /* ignore */ }
}
function scheduleDispatch() {
    if (typeof queueMicrotask === 'function')
        queueMicrotask(dispatchPointerLockChange);
    else
        Promise.resolve().then(dispatchPointerLockChange);
}
/** Install `requestPointerLock` on a class prototype (typically
 * `LiveElement.prototype`). Idempotent. */
function installRequestPointerLockOn(proto) {
...
export function installPointerLockOnDocumentShim(shim) {
    ...
    if (typeof disp === 'function') {
        activeShim = shim;
        activeShimDispatcher = (ev) => disp.call(shim, ev);
    }
}
let installed = false;
/**
 ...
 */
export function installPointerLock(LiveElementCtor) {
    if (installed)
        return;
    installed = true;
...
//# sourceMappingURL=pointer-lock.js.map