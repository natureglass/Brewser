...
import { getLiveRoot, getLiveViewport, getLiveWindow, hitTestLive, pageHasListenerFor, } from '../scripts/live-dom.js';
import { getComputedLiveStyle } from '../scripts/live-css.js';
import { getButtonIndexForAction } from './button-router.js';
import { abortLivePress, beginLivePress, dispatchChromeTap, endLivePress, } from './live-input-dispatch.js';
...
export function setCursorIdleMs(ms) {
...
let chromeY0 = 0;
let chromeY1 = 56;
let chromeMode = 'normal';
export function setMouseChromeRegion(y0, y1) {
    chromeY0 = y0;
    chromeY1 = y1;
}
export function setMouseChromeMode(mode) {
    chromeMode = mode;
}
function cursorInChromeZone() {
    if (chromeMode !== 'normal')
        return false;
...
let activeMousePress = null;
let prevButtons = { a: false, b: false, zr: false };
...
export function installPageMouseForwarder() {
    if (installed)
        return;
    installed = true;
...
function isButtonPressed(pad, idx) {
    return Boolean(pad?.buttons[idx]?.pressed);
}
function readStickAxis(pad, axis) {
 ...
 * defend against). */
export function syncMouseButtonsToCurrent() {
    const pad = activePad();
...
export function tickCursorMovementOnly() {
    const pad = activePad();
...
export function tickMouseInput() {
    const pad = activePad();
    const now = performance.now();
    const dxNorm = readStickAxis(pad, LEFT_STICK_X_AXIS);
    const dyNorm = readStickAxis(pad, LEFT_STICK_Y_AXIS);
    ...
    if (diagTickCount <= 30 || now - diagLastLogAt > 2000) {
        ...
        console.debug('[page-mouse-fwd] tick=' + diagTickCount
    ...
    if (aRising || aFalling || bRising || bFalling || zrRising || zrFalling) {
        console.debug('[page-mouse-fwd] button edge'
    ...
    if (aRising) {
        ...
        if (activeMousePress) {
            ...
            if (hit)
                dispatchPointerOnTarget(hit, 'pointerup', MOUSE_BUTTON_LEFT);
...
function currentButtonsMask() {
    let m = 0;
    if (cursor.buttonsDown.left)
        m |= BUTTONS_MASK_LEFT;
    if (cursor.buttonsDown.right)
        m |= BUTTONS_MASK_RIGHT;
    if (cursor.buttonsDown.middle)
        m |= BUTTONS_MASK_MIDDLE;
    return m;
}
...
function hitElementUnderCursor() {
    try {
...
function buildMouseEvent(type, button) {
    return {
        type,
        clientX: cursor.x,
        clientY: cursor.y,
...
function buildPointerEvent(type, button) {
    return {
...
function dispatchMouseButtonOnWindow(type, button, target) {
    const win = getLiveWindow();
    const pointerType = type === 'mousedown' ? 'pointerdown'
        : type === 'mouseup' ? 'pointerup'
            : null;
 ...
 * see the same input. */
function dispatchPointerOnTarget(target, pointerType, button) {
    const pEv = buildPointerEvent(pointerType, button);
    pEv.target = target;
    pEv.currentTarget = target;
    target.dispatchEvent(pEv);
...
function applyHoverElementCursor(el) {
    if (!el)
        return;
    try {
...
function dispatchTo(el, type, button) {
 ...
 */
export function setCursorFromCss(value) {
...
async function loadCursorSprite(url, hotX, hotY) {
    try {
        ...
        const sprite = { image: img, hotX, hotY, width: w, height: h, url };
...
let cursorOffscreen = null;
let cursorOffscreenW = 0;
let cursorOffscreenH = 0;
let lastBitmapSpec = null;
...
function ensureCursorOffscreen(w, h) {
...
export function syncCursorOverlay() {
    ...
    if (!screenIntrospectionLogged) {
        ...
        console.debug('[page-mouse-fwd] screen introspection'
            ...
            + ' protoCtor=' + protoName
            + ' typeof.setCursorOverlay=' + typeof screen.setCursorOverlay
            + ' typeof.clearCursorOverlay=' + typeof screen.clearCursorOverlay
    ...
    if (needsRender) {
        ...
        if (spec.kind === 'default') {
            ...
            try {
                octx.drawImage(sprite.image, 0, 0, sprite.width, sprite.height);
            }
            catch (_) {
...
function drawDefaultArrow(ctx, x, y) {
    ctx.save();
