...
import { dispatchPageKeyEvent, getInternalLiveScrollY, getLiveRoot, getLiveWindow, hitTestLive, pageHasListenerFor, setInternalLiveScrollY, setInternalLiveViewport, } from '../scripts/live-dom.js';
...
export function setFullscreenVideo(el) {
...
let chromeY0 = 0;
let chromeY1 = 56;
export function setChromeRegion(y0, y1) {
    chromeY0 = y0;
    chromeY1 = y1;
...
export function setStarEnabled(enabled) {
    starEnabled = enabled;
...
export function peekPendingInput() {
    return pendingInput;
}
...
let pendingFrameTap = null;
function handleVideoFrameTap(el) {
...
let pendingActionTap = null;
function handleDoubleTapAction(el, action) {
    const now = performance.now();
    if (pendingActionTap &&
...
let lastFsCanvasTap = null;
function fullscreenCanvasDoubleTap(x, y) {
    const now = performance.now();
    if (lastFsCanvasTap &&
...
export function setNavigating(v) {
    isNavigating = v;
    if (v)
        pendingInput = null;
}
 ...
 */
let liveViewport = { x: 0, y: 0, width: 0, height: 0 };
let liveScrollY = 0;
export function setLiveViewport(v, scrollY = 0) {
    liveViewport = v;
    liveScrollY = scrollY;
...
let touchScrollHandler = null;
export function setTouchScrollHandler(fn) {
...
function findScrollableAncestor(el) {
    for (let n = el; n; n = n.parent) {
        ...
        if (!lb || lb.intrinsicContentH <= lb.contentH)
            continue;
...
function findChromelessVideoAncestor(el) {
    for (let n = el; n; n = n.parent) {
        if (n.tagName === 'VIDEO' && !n.hasAttribute('controls'))
            return n;
    }
    return null;
}
export function installCanvasTouch() {
    ...
    setLiveInputIntentSink((intent) => pushInput(intent));
    const canvas = nxScreen();
    canvas.addEventListener('touchstart', (event) => {
        _touchDiag('touchstart event received');
        const touch = event.touches[0] ?? event.changedTouches[0];
        if (!touch) {
            _touchDiag('  → no touch object, returning');
            return;
        }
        ...
        if (browserMode === 'video-fullscreen') {
            ...
            if (fv) {
                const canvas = nxScreen();
                const ctrl = hitTestVideoControls(0, 0, canvas.width, canvas.height, x, y, fv);
                if (ctrl) {
                    if (ctrl.kind === 'play') {
    ...
    canvas.addEventListener('touchmove', (event) => {
        ...
        if (liveScrollSession) {
            ...
            const target = Math.max(0, Math.min(liveScrollSession.maxScroll, liveScrollSession.startScrollTop - dy));
            liveScrollSession.element.scrollTop = target;
            if (Math.abs(dy) > SWIPE_MOVE_THRESHOLD)
                liveScrollSession.moved = true;
        }
    ...
    canvas.addEventListener('touchend', (event) => {
        ...
        if (kbSwipeSession) {
            ...
            if (!wasSwipe) {
                const globals = globalThis;
                if (globals.__brewserKeyboardCancel)
...
export function setTouchDebugEnabled(enabled) {
    _touchDebugEnabled = enabled;
}
function _touchDiag(label) {
    if (!_touchDebugEnabled)
        return;
    try {
        const sw = globalThis.Switch;
        if (sw?.appendFileSync) {
            sw.appendFileSync(_TOUCH_DIAG_PATH, (Date.now() - _touchDiagStart) + 'ms\tTOUCH: ' + label + '\n');
        }
    }
    catch { /* swallow */ }
}
...
function publishDpadDebug(snap, nowMs) {
    const dbg = {
...
let prevPressedByIdx = new Array(GAMEPAD_BUTTON_COUNT).fill(false);
let nowPressedByIdx = new Array(GAMEPAD_BUTTON_COUNT).fill(false);
function samplePressedByIdx(pad, out) {
 ...
 * fired this tick.
 */
function checkShellRisingEdge(_pad, prev, now) {
    for (const m of listMappedButtons()) {
        if (m.idx < 0 || m.idx >= GAMEPAD_BUTTON_COUNT)
            continue;
...
function shellActionToControllerInput(action) {
    switch (action) {
        ...
        case 'scrollDown':
        case 'scrollLeft':
        case 'scrollRight': return null;
        case '': return null;
    }
}
export async function waitForControllerInput(options = {}) {
    let prev = snapshot();
    ...
    while (true) {
        ...
        if (options.onScroll) {
            ...
            if (pageWantsKeys) {
                ...
                else if (nowMs - rightStickHeldSince >= DPAD_HOLD_DELAY_MS &&
            ...
            if (rising(prev, next, 'dpadUp')) {
                ...
                else if (nowMs - dpadUpHeldSince >= DPAD_HOLD_DELAY_MS &&
            ...
            if (rising(prev, next, 'dpadDown')) {
                ...
                else if (nowMs - dpadDownHeldSince >= DPAD_HOLD_DELAY_MS &&
...
//# sourceMappingURL=controller-shortcuts.js.map