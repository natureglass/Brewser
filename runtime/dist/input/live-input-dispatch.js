...
export function setLiveInputIntentSink(fn) {
    intentSink = fn;
}
...
export function setChromeTapRegion(y0, y1) {
    chromeY0 = y0;
    chromeY1 = y1;
}
export function setChromeTapStarEnabled(enabled) {
    starEnabled = enabled;
}
...
export function pointInChromeStrip(_x, y) {
...
const TOOLBAR_INTENT_BY_ACTION = {
    back: { kind: 'back' },
 ...
 * URL-bar default. */
function findToolbarActionAncestor(el) {
...
function hitTestToolbarAt(root, x, y) {
...
export function dispatchChromeTap(x, y) {
    if (!pointInChromeStrip(x, y))
        return false;
    if (!intentSink) {
        playClick();
        return true;
...
// ----- Press dispatch --------------------------------------------------------
function buildBaseEvent(x, y, buttons) {
    return {
        clientX: x, clientY: y,
        pageX: x, pageY: y,
        screenX: x, screenY: y,
...
export function beginLivePress(el, x, y, source) {
...
export function endLivePress(handle, x, y, opts = {}) {
    const el = handle.el;
...
function resolveIntentAndFormTap(el, x, y) {
    ...
    if (intent.kind === 'video-control') {
        ...
        if (c.kind === 'play')
            return { unhandled: null, shellIntent: { kind: 'video-play', video: intent.video } };
        if (c.kind === 'pause')
            return { unhandled: null, shellIntent: { kind: 'video-pause', video: intent.video } };
...
function runResolvedIntent(resolved) {
    if (!resolved.shellIntent)
        return;
    if (!intentSink)
        return;
    intentSink(resolved.shellIntent);
}
function scheduleClear(handle) {
    if (handle.cleared)
        return;
    const elapsed = performance.now() - handle.pressedSince;
...
function performClear(handle) {
    ...
    for (const n of handle.chain)
        setPseudoActive(n, false);
...
export function abortLivePress(handle) {
    performClear(handle);
}
//# sourceMappingURL=live-input-dispatch.js.map