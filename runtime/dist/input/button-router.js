// Central button-action router. The runtime's controller-shortcuts
// rising-edge dispatcher and the software-cursor layer in
// `page-mouse-forwarder.ts` both consult THIS module to decide what
// each joycon button does. Without a single source of truth the two
// layers were racing — pressing Switch's A button (gamepad index 1)
// fired BOTH the shell's old "back" action (which had been wired via
 ...
 */
export function setButtonMapping(userMapping) {
    for (const label of ALL_LABELS) {
    ...
    for (const label of ALL_LABELS)
        baseMapping[label] = mapping[label];
    if (appOverlayActionKeyed)
        applyActionKeyed(appOverlayActionKeyed);
    rebuildActionToLabelIndex();
...
export function setAppButtonOverlay(overlay) {
...
export function clearAppButtonOverlay() {
    if (!appOverlayActionKeyed)
        return;
...
function rebuildEffective() {
    for (const label of ALL_LABELS)
        mapping[label] = baseMapping[label];
    if (appOverlayActionKeyed)
        applyActionKeyed(appOverlayActionKeyed);
    rebuildActionToLabelIndex();
    console.debug('[button-router] mapping(effective)=' + JSON.stringify(mapping));
}
function applyActionKeyed(userMapping) {
    ...
    const defaultLabelForAction = new Map();
    for (const label of ALL_LABELS) {
    ...
    for (const key of Object.keys(userMapping)) {
        ...
        if (raw === '') {
            ...
            if (!defLabel)
                continue; // action has no default → leave unbound
            label = defLabel;
        }
        else {
            const candidate = raw.toUpperCase().replace(/[\s-]/g, '_');
            if (!(candidate in BUTTON_INDEX_MAP))
                continue;
            label = candidate;
        }
...
export function hasAnyMouseBinding() {
...
export function listMappedButtons() {
    const out = [];
    for (const label of ALL_LABELS) {
        const action = mapping[label];
        if (!action)
...
//# sourceMappingURL=button-router.js.map